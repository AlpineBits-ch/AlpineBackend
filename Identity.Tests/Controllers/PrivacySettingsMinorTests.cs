...
/// <summary>
/// T1-11 over real HTTP: the minor protections as a client experiences them.
///
/// <para>The birth date is set through registration, exactly as a real account's is, so what these
/// exercise is the same derivation the endpoint performs on every request - not a flag some test
/// helper flipped.</para>
/// </summary>
[TestFixture]
public class PrivacySettingsMinorTests
{
    ...
    public async Task Get_AsAMinor_ReportsTheFlooredValuesEvenWhenTheStoredRowIsWider()
    {
        ...
        // account's birth date said what it says now, or by a path that predates the floors. The
        // endpoint must not report it as though it were in effect.

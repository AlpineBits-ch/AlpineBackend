...
/// <summary>
/// Hand-rolled no-op IMessageBus for handler/service unit tests that need to satisfy a bus
/// parameter but don't exercise the transport itself - mirrors this repo's no-mocking-framework
/// convention (see Social.Tests.Helpers.FakeMessageBus).
/// </summary>
public class FakeMessageBus : IMessageBus
{
    public List<object> Published { get; } = new();
    private readonly Dictionary<Type, object> _responders = new();
    public object? LastInvoked { get; private set; }

    public void RespondWith<TRequest, TResponse>(TResponse response) where TRequest : notnull =>
        RespondWith<TRequest, TResponse>(_ => response);

    /// <summary>A responder keyed off the request lets a paged fake answer differently per call,
    /// which a single fixed response cannot.</summary>
    public void RespondWith<TRequest, TResponse>(Func<TRequest, TResponse> responder) where TRequest : notnull
    {
        _responders[typeof(TRequest)] = (Func<object, object?>)(request => responder((TRequest)request));
    }
    ...
    {
        LastInvoked = message;
        if (_responders.TryGetValue(message.GetType(), out var responder))
        {
            var factory = (Func<object, object?>)responder;
            return Task.FromResult((T)factory(message)!);
        }

        throw new InvalidOperationException($"No fake response registered for {message.GetType()}");
    }

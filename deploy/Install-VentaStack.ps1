...
if (-not $ReuseEnv) {
    ...
    switch ($Config['TLS_MODE']) {
        { $_ -in 'letsencrypt', 'external-proxy' } {
            ...
            $Config['WIKI_DOMAIN'] = Format-EnvValue (
                Get-Setting -Preset $WikiDomain -Prompt 'Public hostname for published guild wikis' `
                            -Default "wiki.$($Config['INSTANCE_DOMAIN'])")
...
WIKI_DOMAIN="$($Config['WIKI_DOMAIN'])"
...
# Generated by deploy\Install-VentaStack.ps1 - edited copies are overwritten on re-run.
{
	...
	# Each published guild wiki answers on a hostname of its own beneath the wiki domain. A wildcard
...
# credential. A DNS record has to cover them: *.$($Config['WIKI_DOMAIN']), pointing at this host.
*.$($Config['WIKI_DOMAIN']) {
	encode zstd gzip
...
"@
    ...
    if ($Config['TLS_MODE'] -eq 'external-proxy') {
        ...
        Write-Host ''
        Write-Host '  The gateway also serves six sites, each on its own hostname and nowhere else, all'
        Write-Host '  on the same 127.0.0.1:8080. Send them there too, preserving the Host header:'
        Write-Host ''
        Write-Host "    $($Config['DOCS_DOMAIN']) $($Config['ADMIN_DOMAIN']) $($Config['SUPPORT_DOMAIN']) $($Config['STATUS_DOMAIN']) $($Config['AUTH_DOMAIN']) $($Config['WIKI_DOMAIN'])"
        Write-Host ''
        ...
        Write-Host ''
        Write-Host "  $($Config['WIKI_DOMAIN']) is the one that is more than one name: each published guild wiki"
        Write-Host "  answers on <slug>.$($Config['WIKI_DOMAIN']), so send *.$($Config['WIKI_DOMAIN']) to the same place."
        Write-Host '  Your proxy needs a certificate covering those names - a wildcard, or per-name issuance'
        Write-Host '  gated on http://127.0.0.1:8080/api/v1/wiki/certificate-check?domain=<name>, which answers'
        Write-Host '  200 only for a wiki some guild has actually published. Until that certificate exists, do'
        Write-Host '  not advertise the per-wiki names: a browser meets a TLS warning rather than a missing'
        Write-Host "  page. The $($Config['WIKI_DOMAIN'])/<slug>/<page> form keeps working and redirects."
        Write-Host ''

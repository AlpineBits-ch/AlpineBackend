...
/// EchoTestStack) fires a real AccountPurgeStartedEvent over RabbitMQ, Echo's
/// AccountDeletionSaga fans PurgeUserDataCommand out to Identity/Social/Guild/Messaging/
/// Federation/Import as real separate processes, and each one's handler does its real Postgres
/// write. Nothing here is mocked or bypassed with a test-only trigger endpoint - the same code
/// path a production 30-day grace period would eventually take is what runs, just compressed to
/// single-digit seconds.
///
/// Bots and Isle are deliberately not exercised (or spawned - see EchoTestStack) since they
/// aren't part of this iteration's fan-out; see AccountDeletionSaga's doc comment for why.
///
/// Sets up one guild (owned by the deleted user, with a second member so ownership-transfer has
/// somewhere to go), one accepted friendship, and one DM message before triggering deletion, so
/// a single test run proves every purge handler's real behavior against real data rather than
/// each in isolation against an empty account.
/// </summary>
[TestFixture]
[Category("E2E")]

...
/// <summary>
/// Applies an imported Discord guild's onboarding and welcome screen. Routed through
/// OnboardingConfigService like every other write, so an imported prompt cannot smuggle in a role
/// the first-party endpoint would have rejected - Discord servers routinely hand out roles through
/// onboarding that carry moderation permissions here.
/// </summary>
public class ImportGuildOnboardingHandler
{
        ...
        MicroserviceContext ctx, OnboardingConfigService configService)
    {
        ...
        if (request.Onboarding is not null)
        {
            ...
            // Discord allows an option that grants nothing; we don't. Drop those (and any prompt
            // left empty) rather than failing the whole import on someone else's config.

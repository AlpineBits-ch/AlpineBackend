...
/// <summary>
/// Covers T2-21's enforcement point. There is no clip feature, so almost every test here is a
/// refusal - that is the point of the item. The one "allows" case exists to prove the check is not
/// a hard-coded <c>false</c>: it has to be possible to pass, or the seam would not actually be
/// enforcing anything when a clip feature is eventually built against it.
///
/// <para>The shape being asserted is the one the spec insists on: per session, per participant, at
/// record time. An account-level flag on its own never authorises recording, so
/// <see cref="StubSessionConsentStore"/> stands in for the per-session prompt a real feature would
/// have to add.</para>
/// </summary>
[TestFixture]
public class VoiceRecordingConsentTests
{
    ...
    {
        // No clip feature exists, so no per-session grant can exist either. The seam is closed by
        // construction: the only registered consent store grants nothing, and enabling recording
        // means replacing it - there is no route that skips the grant.
        SeedCall("user-1", "user-2");

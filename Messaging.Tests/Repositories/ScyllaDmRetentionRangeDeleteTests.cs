...
/// <summary>
/// T2-22's retention range read and the deletes it feeds, <b>against a real Scylla node</b>.
///
/// <para><b>Why this cannot be a fake-mapper test.</b> The retention scan is the only statement in
/// this service that mixes relation forms on one clustering column -
/// <c>(created_at, message_id) &gt; (?, ?)</c> for the resume cursor against <c>(created_at) &lt; (?)</c>
/// for the window - because Cassandra refuses a multi-column and a single-column relation on the
/// same column, so the upper bound has to be written as a one-component tuple. FakeCassandraMapper
/// records the text of a statement; it cannot reject one, cannot enforce clustering order, and
/// cannot tell an operator-precedence mistake from a correct query. Everything it proved about this
/// statement would have been equally true of a statement the cluster refuses - or, far worse, of one
/// the cluster accepts and which selects the wrong rows. <b>What this query selects is what gets
/// deleted, and deleted messages do not come back.</b></para>
///
/// <para><b>Every meaningful assertion here is a negative one.</b> Newer rows survive. The other
/// party's rows survive. Other partitions survive. An empty window deletes nothing. A retention job
/// that deletes slightly too little is a bug; one that deletes slightly too much is an incident.</para>
///
/// <para>Skipped, loudly and by name, when no node is configured - the same shape as
/// <c>MlsConcurrentCommitPostgresTests</c> and <c>ECHO_TEST_POSTGRES</c>, so CI without the
/// container still passes. Point <c>ECHO_TEST_SCYLLA</c> at a node
/// (<c>host:port</c>, e.g. <c>localhost:9042</c>) to run it; the compose file in the repository root
/// already stands one up. <c>ECHO_TEST_SCYLLA_USERNAME</c>/<c>_PASSWORD</c> are optional and only
/// needed for a node started with PasswordAuthenticator, as compose.yaml's is.</para>
///
/// <para>This suite is what <c>RETENTION_DM_SCYLLA_ENABLED</c> gates on: the delete path ships off
/// by default and a deployment turns it on after running these against a node of the version it
...
public class ScyllaDmRetentionRangeDeleteTests
{
    ...
    /// <summary>One partition per test. Cheaper and far less flaky than a keyspace per test, and the
    /// retention query is partition-scoped anyway, so tests cannot see each other's rows.</summary>
    ...
    public async Task TheRangeReadComesBackOldestFirst()
    {
        ...
        // resume cursor moves forward through history. If that override were dropped, the cursor
        // would walk backwards and the scan would re-read its first page forever.
        var contextId = NewContextId();
    ...
    {
        // A user whose messages are all newer than their retention window. The scan must come back
        // empty rather than, say, ignoring an upper bound the cluster silently did not apply.
        var contextId = NewContextId();
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // What the delete must NOT touch
    // ══════════════════════════════════════════════════════════════════════════
    ...
    {
        // The partition key is the only thing scoping the delete. A missing or wrong context_id
        // predicate would be a cross-conversation delete, which is the worst outcome available here.
    ...
    public async Task PagingTheRangeNeitherSkipsNorRepeatsSameMillisecondSiblings()
    {
        ...
        // message_id. A cursor on the timestamp alone would re-emit the whole millisecond or skip
        // all of it; on a *delete* path, skipping it means those messages are never removed and the
        // user's retention setting silently under-delivers forever.
        var contextId = NewContextId();
        var sameInstant = Now - TimeSpan.FromDays(40);
    ...
    {
        // The regression this whole fixture was written to catch, in its end-to-end form: a page cap
        // that lands in the middle of a same-millisecond group. Under the original row-tuple cursor
        // the sweep deleted the first row of the group and abandoned the rest on every tick forever,
        // and the other party's row at the same instant was the only thing that made the shortfall
        // visible at all. Page size 2 against a group of 5 forces the boundary inside the group.
        var contextId = NewContextId();
        var sameInstant = Now - TimeSpan.FromDays(40);

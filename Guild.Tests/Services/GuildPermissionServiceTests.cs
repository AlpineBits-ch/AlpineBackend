...
public class GuildPermissionServiceTests
{
    ...
    {
        // Role bits are resolved from role_members, which is keyed by member id alone. A row
        // pointing at a foreign guild's role would otherwise contribute that role's permissions
        // here - which is what turned a cross-guild role assignment into Superadmin in the victim
        // guild. The query is guild-scoped so no such row can matter, whatever created it.
        _context.Guilds.Add(MakeGuild());
    ...
    public async Task CanUserPerformAction_GuildOwner_AlwaysReturnsTrue()
    {
        ...
        // MakeGuild seeds the Community preset, so the household-module permissions are
        // deliberately absent - a feature being off is a product state, not an authorization
        // level, so ownership does not override it (see GuildFeatureGateTests).
    ...
    // Each test grants exactly one "root" permission via a base role and then
    // asserts the permissions that must be implied by it.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    {
        // A ChannelPermission that has a RoleId but neither a ChannelId nor a
        // CategoryId was the old way to express base role permissions.  In the
        // Discord-like model, base permissions live on Role.Permissions directly,
        // so these legacy records must have no effect.
        _context.Guilds.Add(MakeGuild());
        _context.Channels.Add(MakeChannel());
        _context.Roles.Add(MakeRole(permissions: Permissions.None));
        _context.GuildMembers.Add(MakeGuildMember());
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // Member-level guild permission overrides
    // GuildMember.AllowPermissions / DenyPermissions sit between role aggregation
    // and channel/category overwrites in the resolution stack.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    public async Task MemberAllow_TakesPrecedenceOverRoleOnGuildCheck()
    {
        ...
        // Guild-level check (no channel context) must honour the member override.
        _context.Guilds.Add(MakeGuild());
        _context.Roles.Add(MakeRole(permissions: Permissions.None));
    ...
    public async Task MemberDeny_TakesPrecedenceOverRoleOnGuildCheck()
    {
        ...
        // Guild-level check must honour the member deny.
        _context.Guilds.Add(MakeGuild());
        _context.Roles.Add(MakeRole(permissions: Permissions.SendMessages));
        _context.GuildMembers.Add(MakeGuildMember(deny: Permissions.SendMessages));
        _context.RoleMembers.Add(MakeRoleMember("rm-1", RoleId, MemberId));
    ...
    public async Task MemberAllow_NotOverriddenByChannelRoleDeny_WhenMemberSpecificAllowPresent()
    {
        ...
        // The channel role-deny should not touch the final result because the member-specific
        // channel allow (highest priority) re-grants the permission.
        _context.Guilds.Add(MakeGuild());
        _context.Channels.Add(MakeChannel());
        _context.Roles.Add(MakeRole(permissions: Permissions.None));
        _context.GuildMembers.Add(MakeGuildMember(allow: Permissions.SendMessages));
    ...
    public async Task OnboardingPending_ButOnboardingDisabled_DoesNotStripPermissions()
    {
        ...
        // while it was on and an admin turned it off before they got around to accepting. Gating
        // on OnboardingCompletedAt alone would leave them permanently unable to participate with
        // no rules screen left to accept.
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // ClampToGrantableAsync / CanGrantPermissionsAsync — bot-install privilege clamp
    // Extracted so the install flow can silently downgrade an over-broad request
    // instead of rejecting it outright; CanGrantPermissionsAsync must keep its
    // original boolean semantics on top of the same shared math.
    // ══════════════════════════════════════════════════════════════════════════

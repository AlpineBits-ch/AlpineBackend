...
/// <summary>
/// <c>api/v1/admin/dsr</c> over real HTTP, real auth and real Postgres (T1-13).
///
/// <para>The negative cases carry the weight here. This queue is a list of named individuals'
/// addresses and the rights they are exercising, so "an ordinary authenticated account cannot see it"
/// is the assertion that matters most - and "a request cannot be closed without saying how it was
/// answered" is the one that stops the queue from becoming a way to make work disappear.</para>
/// </summary>
[TestFixture]
public class AdminDsrControllerTests
{
    ...
    public async Task Open_WithABackdatedArrival_ShortensTheRemainingWindow()
    {
        ...
        // Ten of the thirty days were already gone by the time it reached the queue. The range is
        // 19-20 rather than a single number because the remaining window is floored and the arrival
        // time is a few milliseconds behind the moment the row is stamped - which is exactly the
        // point being made: the clock does not restart at data entry.

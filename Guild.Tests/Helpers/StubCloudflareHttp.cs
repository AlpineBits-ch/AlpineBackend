...
/// <summary>
/// Builds a real <see cref="CloudflareService"/> over a canned HTTP handler.
///
/// CloudflareService is a concrete class with non-virtual methods, so it cannot be substituted
/// directly; it takes an <see cref="IHttpClientFactory"/>, which is the seam. Tests that only care
/// about our own bookkeeping around a session still need session creation to return *something*.
/// </summary>

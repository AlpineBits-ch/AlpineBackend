    ...
    ILogger<SteamAuthenticationController> logger) : ControllerBase
{
    ...
    /// <summary>
    /// Shared return endpoint Steam redirects the browser to. Verifies the assertion and then
    /// either links the SteamID to the target user or mints a one-time login ticket. Always ends
    /// on a browser redirect to the configured client return URL with a ?status= param.
    /// </summary>
    [AllowAnonymous]
    [HttpGet("callback")]

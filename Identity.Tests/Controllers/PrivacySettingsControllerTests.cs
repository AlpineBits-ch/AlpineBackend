...
///
/// <para>The endpoint is thin - <see cref="Identity.Application.Services.PrivacySettingsPatch"/> is
/// unit-tested on its own - so what is asserted here is everything the applier cannot see: that a
/// write survives the round trip to the database, that it bumps the version and leaves an audit row,
/// that a refused write leaves neither, and that an unauthenticated caller cannot read the record at
/// all.</para>
/// </summary>
[TestFixture]
public class PrivacySettingsControllerTests
{
    ...
    {
        // Version is the server's monotonic counter. A client that could set it could replay an old
        // state past every consuming cache's freshness check.

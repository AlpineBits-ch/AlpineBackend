...
/// CallParticipantLeftHandler, CallRingTimeoutCheckHandler, CallWentAloneHandler). Every test that
/// can reach CallEndNotifier/CallPushService's cancel-recipients branch is deliberately shaped so
/// that branch's recipient list ends up empty (either a single-participant call where the acting
/// user is the only participant, or a call with no participants at all) - CallPushService wraps a
/// static ApnsClient field whose initializer touches real Apple/Firebase credentials the first
/// time the type is touched with a non-empty token list, which isn't something to risk exercising
/// from a unit test. CallAloneTimeoutCheckHandler's actual "ends the call" branch is left uncovered
/// for the same reason: EndIfStillAlone only fires End() with exactly one connected participant,
/// which unavoidably makes cancelRecipientIds non-empty. CallDeviceTakeoverHandler's Cloudflare
/// close-tracks branch is left uncovered too - real HTTP via CloudflareService, no fake seam.
/// </summary>
[TestFixture]

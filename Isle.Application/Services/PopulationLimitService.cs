    ...
    ILogger<PopulationLimitService> logger) : BackgroundService
{
    ...
    private async Task SafeEnforceAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            // _lastState is left untouched, so the next tick retries from scratch.
    ...
    private async Task EnforceAsync()
    {
        ...
        if (!rcon.IsConnected)
        {
            // GetPlayerData/UpdatePlayables both require a live connection.
        ...
        foreach (var player in players)
        {
            ...
            {
                // FriendlyName falls back to the raw class when it can't map it.
                logger.LogWarning(
        ...
        // sent (format: "class:enabled,class:disabled,...").

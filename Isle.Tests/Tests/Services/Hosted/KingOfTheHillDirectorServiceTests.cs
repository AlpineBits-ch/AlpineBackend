...
/// <summary>
/// <see cref="KingOfTheHillDirectorService"/> waits <c>StartupDelay</c> (40s) before its first tick and
/// then delays <c>Interval</c> (30s) between ticks, so a short lifecycle test can only ever exercise the
/// startup-cancellation path. <c>SafeTickAsync</c> is `internal` for exactly this reason: it lets these
/// tests drive individual ticks directly. The branches that would require standing up the full
/// KingOfTheHillDirector/Spawner/TickService/CompletionService graph (already covered by their own
/// dedicated test fixtures — KingOfTheHillDirectorTests, KingOfTheHillTickServiceTests,
/// KingOfTheHillCompletionServiceTests) are deliberately left to those fixtures; this file focuses on the
/// wrapper's own dispatch/guard logic.
/// </summary>
[TestFixture]

...
/// <summary>
/// <see cref="InviteTimeoutService"/>'s loop delays for <c>Interval</c> (20s) before every tick, so a
/// short-lived <c>StartAsync</c>/<c>StopAsync</c> lifecycle test would never actually run
/// <c>ExpireStaleInvitesAsync</c> — it would only ever exercise the cancellation path. Instead this
/// drives the tick logic directly (it's `internal` for exactly this purpose) against a real SQLite
/// context, and covers the hosted wrapper itself with a thin smoke test.
/// </summary>
[TestFixture]

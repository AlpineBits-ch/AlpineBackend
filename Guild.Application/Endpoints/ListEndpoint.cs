...
/// <summary>Items on a <see cref="ChannelType.List"/> channel - the shopping list and the todo
/// list, which are the same rows rendered differently.
///
/// Every mutation broadcasts, because the defining use case is two people in the same shop: if one
/// ticks off milk the other's phone has to strike it out within the second, or they buy it twice.
///
/// Wolverine's DbContext middleware does not apply to these (they're HTTP endpoints, not bus
/// handlers), so each commits explicitly.</summary>
[Authorize]
public class ListEndpoint
{
        ...
        [NotBody] ClaimsPrincipal user)
    {
        ...
        // Append to the end of the unchecked block. Max+1 rather than count so re-ordering never
        ...
        [NotBody] ClaimsPrincipal user)
    {
        ...
        // action. Shopping lists are collaborative, but silently rewriting what a flatmate asked
        // for is not.
        ...
        [NotBody] MicroserviceContext ctx, [NotBody] ClaimsPrincipal user)
    {
        ...
        // pantry can put it back on the list next time it's low. Without this, deleting the line
        // instead of buying the milk means the pantry never asks again.

...
/// <summary>Stock tracking on a <see cref="ChannelType.Pantry"/> channel. The location is the
/// channel (Fridge, Freezer, Cellar), so per-location visibility is just channel permissions.
///
/// The interesting behaviour is the restock loop - see <see cref="PantryRestockService"/>.</summary>
[Authorize]
public class PantryEndpoint
{
    ...
    /// <summary>The "eat me first" board: what's about to go off, soonest first. Spans every
    /// pantry channel in the guild the caller can see, because "what needs eating" is a question
    /// about the house, not about the fridge specifically.</summary>
        ...
        [NotBody] MicroserviceContext ctx, [NotBody] ClaimsPrincipal user)
    {
        ...
        // Restocked back above the threshold: release the stamp so the next dip re-adds it. This
        // is the other half of the idempotency guard.

...
/// <summary>
/// Hand-rolled no-op IMessageBus for handler unit tests that need to satisfy a bus parameter but
/// don't exercise any bus behavior themselves - mirrors this repo's no-mocking-framework
/// convention. Only PublishAsync actually does anything (records calls for assertions); every
/// other member throws since nothing in this test suite should ever reach them.
/// </summary>
public class FakeMessageBus : IMessageBus
{
    ...
    public Task<T> InvokeAsync<T>(object message, CancellationToken cancellation = default, TimeSpan? timeout = null) => throw new NotImplementedException();

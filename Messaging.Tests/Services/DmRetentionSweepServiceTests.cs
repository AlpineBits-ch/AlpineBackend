...
public class DmRetentionSweepServiceTests
{
    ...
    private async Task<Message> SeedMessage(string conversationId, string authorId, int daysOld)
    {
        ...
        // context that is not already tracking those rows. In production every tick gets a fresh
        // scope and therefore a fresh context; this fixture shares one, so the seeding has to hand
        // it back in the same state.
    ...
    /// modelled on does. The test container hands out the one in-memory context so assertions can be
    /// made against the same store the sweep wrote to.
    /// </summary>
    ...
    {
        // The line between a retention control and a history-rewriting one. The other person's
        // copy of a conversation they were also in is not this user's to erase.
    ...
    public async Task DoesNotStallBehindAPageOfSomebodyElsesMessages()
    {
        ...
        // makes no progress on any tick, forever. The cursor advances off the last row read, not
        // off the last row deleted.
        _context.Members.AddRange(
            MakeMember("m-1", "user-1", "conv-1"),
            MakeMember("m-2", "user-2", "conv-1"));
        await _context.SaveChangesAsync();
    ...
    {
        // The defaults a deployment gets if it configures nothing. RETENTION_DM_SCYLLA_ENABLED is
        // the only way in, and it is not set in this test process.
    ...
    {
        // "yes" is not a bool. It must leave the environment's answer alone rather than being
        // coerced to false, or a typo in appsettings turns a deliberately enabled delete path off
        // and nobody finds out until a user asks why nothing was deleted.
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // Defect 1: the rotation. Without a cursor the sweep re-swept the same first
    // MaxUsersPerTick accounts every tick and never reached anybody after them.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    {
        // The set is not stable between ticks. An id that sorts before the current position is not
        // reached in the rotation it appeared during - that is inherent to a forward scan - but the
        // wrap has to reach it in the next one, or "eventually swept" is not true of new accounts.
        _context.Members.AddRange(
    ...
    public void TheRotationPageTranslatesOnTheRealProvider()
    {
        ...
        // process and so cannot fail on a predicate Npgsql has no translation for. The sweep's page
        // query is the one place that matters: an untranslatable comparison throws on every single
        // tick, and the only symptom is a log line nobody is reading and retention that never runs.
        //
        // No connection is opened - ToQueryString compiles the expression tree and stops there.
    ...
    {
        // The default. The range delete has been proved against a live node
        // (ScyllaDmRetentionRangeDeleteTests) but a deployment opts in explicitly, so until it does
        // the sweep must not send a single statement at the cluster.
        _context.Members.Add(MakeMember("m-1", "user-1", "conv-1"));
        await _context.SaveChangesAsync();
    ...
    public async Task TheGateDoesNotApplyToTheRelationalBackend()
    {
        ...
        // Gating it on a Scylla flag would disable retention on every self-hosted deployment.
        _context.Members.Add(MakeMember("m-1", "user-1", "conv-1"));
        await _context.SaveChangesAsync();

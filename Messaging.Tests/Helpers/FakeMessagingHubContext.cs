...
/// <summary>
/// Records not just what was pushed but <i>who it was pushed to</i>. The target matters for MLS: a
/// Welcome is sealed to one device's leaf, so fanning it out to every session the user has open is
/// a real (if quiet) bug, and a test that only inspected the method name could not tell the
/// difference.
/// </summary>

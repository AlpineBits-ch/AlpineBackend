...
/// - covered separately).
///
/// Reacting is authorized against the *target message's* context, not the DTO's - so every test
/// here seeds the message it reacts to, and channel reactions answer a
/// HasUserPermissionToChannelRequest for AddReactions.
/// </summary>
[TestFixture]
public class ReactionsEndpointTests
{
    ...
    /// <summary>Seeds the message being reacted to. Context comes from the message, so it has to
    /// exist for any reaction to be authorized.</summary>

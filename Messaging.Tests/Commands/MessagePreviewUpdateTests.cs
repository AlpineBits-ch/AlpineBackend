...
public class MessagePreviewUpdateTests
{
    ...
    /// <summary>
    /// Commits and detaches between two handler invocations.
    ///
    /// <para>The handler itself never calls SaveChanges - Wolverine's transaction middleware does
    /// that after Handle returns (see MessageDeletedHandler's note on the same convention). A test
    /// driving the handler directly has to stand in for the middleware, or the second call reads
    /// the pre-write row and every assertion about ordering silently tests nothing.</para>
    /// </summary>
    ...
    {
        // The whole reason EditedAt exists. Before the split, posting a URL made your own message
        // say "(edited)" a second later, done by nobody.
        var message = await SeedMessage();
    ...
    {
        // Fetching a page takes seconds; the author can edit the message in that window. Attaching
        // the old link's card to the new text would be wrong with no way to tell.
        var message = await SeedMessage("see https://example.com");
    ...
    {
        // The dismissal and the in-flight unfurl race each other. Whichever order they land in,
        // the human's decision has to win.
        var message = await SeedMessage();
    ...
    public async Task AuthorizationAlreadyChecked_DoesNotLetAStrangerEditText()
    {
        ...
        // check. It must stay a bypass of the *comparison*, not a grant of edit rights - but the
        // comparison is all this handler has, so this test documents that the endpoints are what
        // guard it, and that nothing else in the codebase sets the flag on a content edit.
        var message = await SeedMessage();

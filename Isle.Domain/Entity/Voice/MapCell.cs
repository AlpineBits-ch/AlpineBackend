...
public readonly struct MapCell : IEquatable<MapCell>
{
    ...
    // Audibility uses this block as the coarse membership filter: with CellSize >= the
    // client's attenuation radius, every peer within earshot of any point in this cell is
    // guaranteed to fall inside the block, so the smooth distance falloff — not the grid —
    // owns the audible edge. See VoiceGridConfig.CellSize for the load-bearing invariant.

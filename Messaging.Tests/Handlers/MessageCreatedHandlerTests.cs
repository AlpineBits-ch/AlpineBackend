...
/// MessageCreatedForChannel), plus the standalone LoadAsync helper. The per-recipient push
/// notification loop itself calls PushNotifiaction.SendPushNotification, a thin wrapper over the
/// real Firebase Admin SDK with no seam to fake - every test here supplies an empty device-token
/// response so that loop body never executes, the same accepted-gap pattern as CallPushService/
/// IceServerService/PushNotifiaction elsewhere in this suite.
/// </summary>
[TestFixture]

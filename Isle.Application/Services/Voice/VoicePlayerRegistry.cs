...
/// <summary>
/// Bidirectional steamId &lt;-&gt; playerId(userId) map for voice opt-in. Reads are served from an
/// in-memory cache (the hot path — <c>TryGetPlayerId</c> runs for every position snapshot), while
/// Redis is the durable source of truth so the mapping survives an API restart.
///
/// <para>Each entry carries a sliding 2h TTL. Active players keep refreshing it (see
/// <see cref="TouchAsync"/>, driven off the position stream), so a missed leave event simply falls
/// out of the cache on its own within the window instead of pinning a ghost in the voice grid.</para>
/// </summary>
public sealed class VoicePlayerRegistry(IConnectionMultiplexer redis, ILogger<VoicePlayerRegistry> logger)
{
    ...
    /// Throttled locally so the position hot path only touches Redis once per <see cref="TouchThrottle"/>.
    /// </summary>

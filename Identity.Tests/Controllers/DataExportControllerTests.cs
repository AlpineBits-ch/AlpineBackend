...
///
/// <para>These are the internal paths; the gateway maps <c>/api/v1/identity/**</c> onto
/// <c>/api/v1/**</c>, so a client calls <c>/api/v1/identity/data-exports</c>.</para>
///
/// <para><b>The negative cases carry the weight.</b> An export is the densest bundle of personal data
/// this system can produce, so the assertions that matter are "somebody else's export id is a 404",
/// "a second request inside the window is a 429", and "the download writes an audit row before the
/// URL is handed out". A feature that only works is not the same as a feature that only works for the
/// right person.</para>
/// </summary>
[TestFixture]
public class DataExportControllerTests
{
    ...
    public async Task List_PartialExport_ReportsItAsPartialAndNamesTheMissingServices()
    {
        ...
        Assert.Multiple(() =>
        {
            // Not "Ready". A subject told their export is ready reasonably believes they received
            // everything, and for an Art. 15 response that is a materially different answer.
    ...
    public async Task Request_AfterAPartialExport_IsAllowedImmediately()
    {
        ...
        // A partial export is an internal fault wearing the shape of an answer. Charging it against
        // the one-per-24h window would make our outage cost the subject a statutory day AND leave
        // them holding an incomplete disclosure they were told to wait to re-request.
        var second = await RequestExportAsync(token);
    ...
    public async Task Download_ExportPastItsWindow_Is410EvenBeforeTheSweepHasRun()
    {
        ...
        // Ready, with a window that closed a second ago. The row still says Ready and the object is
        // still in the bucket - the promise is the window, not the sweep's schedule.
    ...
    public async Task Download_PartialExport_StillRedirects302AndIsAudited()
    {
        ...
        // The status is deliberately not Ready, and the download works anyway. A partial archive is
        // still the subject's own data, produced in answer to a statutory request; withholding it
        // because two of eight services were down would turn "some of this is missing" into "you get
        // nothing", which is the one outcome worse than answering short.
        var result = await Host.Scenario(x =>
        {
            x.WithBearerToken(token);
            x.Get.Url($"{Url}/{exportId}/download");
    ...
    public async Task List_DoesNotExposeTheArtifactKey()
    {
        ...
        // The key is a bearer credential for the archive. The only thing that ever hands it out is
        // the download route, and then only as a signed URL that expires in minutes.

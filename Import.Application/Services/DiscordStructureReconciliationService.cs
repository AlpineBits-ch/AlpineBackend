...
/// <summary>
/// Backstop for the live Gateway sync: re-fetches each Active GuildLink's structure via REST
/// and re-applies it through the same upsert path the Gateway dispatches use, plus deletes any
/// mapped entity no longer present on the Discord side. Corrects drift from any event the
/// Gateway connection missed (a gap between disconnect and reconnect, a dropped frame, an
/// Invalid Session) - same periodic-BackgroundService shape as

...
/// <summary>
/// Builds <see cref="PrivacySettingsCache"/> and <see cref="BlockCache"/> over the in-process fakes,
/// so a test can say "Identity says this" or "Social is unreachable" in one line.
///
/// <para>Both caches read through <see cref="FakeDistributedCache"/> and fall back to
/// <see cref="FakeInvokingMessageBus"/>, whose <c>ClearResponses</c> is how a test expresses an
/// unreachable service - which is the case that actually matters here, because every one of these
/// controls has to fail closed.</para>
/// </summary>
internal static class PrivacyTestFactory
{
    ...
    /// <summary>A cache whose bus answers with exactly <paramref name="settings"/>. Any user not
    /// listed comes back restrictive, exactly as it would in production when Identity omits an id.</summary>

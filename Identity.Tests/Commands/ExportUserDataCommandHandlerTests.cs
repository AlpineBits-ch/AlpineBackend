...
/// <summary>
/// Identity's participant in the cross-service export fan-out (T1-7).
///
/// <para>Like the purge handler this is a Wolverine bus handler, so per this repo's convention it
/// never calls <c>SaveChangesAsync</c> itself - the tests here call it afterwards to stand in for the
/// transactional middleware.</para>
///
/// <para><b>The test that matters is the negative one.</b> An export is the densest bundle of personal
/// data this system can produce, and the way this feature becomes a breach is not by failing - it is
/// by succeeding and including one column too many. So there is an explicit assertion that a second
/// account's email address, phone number and session IP do not appear anywhere in the fragment, and
/// another that the subject's own credentials do not either.</para>
/// </summary>
[TestFixture]

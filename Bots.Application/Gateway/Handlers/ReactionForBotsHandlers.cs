...
public class ReactionCreatedForBotsHandler
{
    public static Task Handle(ReactionCreatedForBots evt, MicroserviceContext ctx, GatewayConnectionRegistry registry,
        ...
        ReactionDispatchHelper.DispatchAsync(ctx, registry, visibility, "MESSAGE_REACTION_ADD", evt.GuildId, evt.ChannelId, evt.MessageId, evt.UserId, evt.Emoji);
}
...
public class ReactionRemovedForBotsHandler
{
    public static Task Handle(ReactionRemovedForBots evt, MicroserviceContext ctx, GatewayConnectionRegistry registry,
        IBotChannelVisibility visibility) =>
        ReactionDispatchHelper.DispatchAsync(ctx, registry, visibility, "MESSAGE_REACTION_REMOVE", evt.GuildId, evt.ChannelId, evt.MessageId, evt.UserId, evt.Emoji);
}

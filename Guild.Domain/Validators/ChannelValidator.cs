...
public class ChannelValidator : AbstractValidator<Channel>
{
    public ChannelValidator()
    {
        ...
        // spaces, but Thread names are free-text titles (a forum post's title, a discussion
        // thread's subject) - "Dark mode is too bright" is a completely normal thread name that
        // this rule would otherwise reject.
        RuleFor(x => x.Name)
            .Must(v => !v.Any(char.IsWhiteSpace))
            .WithMessage("Channel name cannot contain whitespace")
            .When(x => x.Type != ChannelType.Thread);
    }
}
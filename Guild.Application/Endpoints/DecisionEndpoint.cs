...
/// <summary>House decisions on a <see cref="ChannelType.Decisions"/> channel.
///
/// This is not a poll. An option is carried when quorum is met and <b>nobody has blocked it</b>;
/// one reasoned block stops it however much support it has. Household questions ("can my cousin
/// stay a fortnight", "are we getting a cat") aren't well served by majority rule - the person who
/// has to live with the downside should be able to stop it, and everyone else should be able to
/// read why. That's why <see cref="DecisionVoteKind.Block"/> requires a reason.</summary>
[Authorize]
public class DecisionEndpoint
{
    ...
    /// <summary>Resolves the decision from the votes cast so far. Anyone who can create decisions
    /// may close one - a house doesn't need a chairperson.</summary>

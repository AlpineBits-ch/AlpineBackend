...
public class HasUserPermissionHandlersTests
{
    ...
    public async Task GuildHandler_EveryExternalPermission_MapsWithoutThrowing(ExternalPermission permission)
    {
        ...
        //
        // IsAllowed is only asserted for permissions this guild's feature set actually offers.
        // SeedOwner creates a Community guild, so the household-module permissions resolve to
        // false by design: the feature gate runs ahead of the owner short-circuit, and ownership
        // does not override a module the guild doesn't have.
        var response = await HasUserPermissionToGuildHandler.Handle(

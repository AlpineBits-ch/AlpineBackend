...
///
/// <para><b>Why this suite exists.</b> The feature shipped with 222 passing unit tests and did
/// nothing whatsoever in the cluster: Messaging never registered its handler, so no message was
/// ever sent to the unfurler and no preview ever appeared. Not one unit test could have caught
/// that, because every interesting step of this feature happens <i>between</i> processes -
/// MessageCreated is raised in Messaging, cascades UnfurlMessageLinks over RabbitMQ, request/
/// responds to a separate service, and comes back as an UpdateMessageCommand whose result reaches
/// clients as a second, later message. The seam is the feature.</para>
///
/// <para>So everything here runs for real: a real HTTP origin serving real Open Graph tags and a
/// real PNG, the real Unfurl process fetching it, real ImageSharp decoding, a real MinIO bucket,
/// and the real Postgres row the preview lands in.</para>
/// </summary>
[TestFixture]
[Category("E2E")]
public class LinkPreviewFlowTests
{
    ...
    /// <summary>
    /// How long a preview is allowed to take to appear.
    ///
    /// <para>Generous on purpose. The path crosses two RabbitMQ hops and an HTTP fetch, and the
    /// unfurler's own fetch timeout is five seconds - a bound tight enough to trip on a loaded CI
    /// runner would turn the central assertion of this suite into a flake, which is worse than a
    /// slow test.</para>
    /// </summary>
    ...
    /// <summary>
    /// Polls history until the message carries at least one embed.
    ///
    /// <para>Polling rather than awaiting a signal because that is exactly the client contract: a
    /// preview is not part of the send, it arrives later as an update, and any consumer has to
    /// tolerate it being absent for a while and then appearing.</para>
    /// </summary>
    ...
    /// <summary>
    /// A message's body as text.
    ///
    /// <para><c>Message.Content</c> is <c>byte[]</c> - it carries MLS ciphertext in encrypted
    /// contexts - so it serializes as base64 and reading it as a plain string silently yields
    /// something that matches nothing. Same decode the other scenario suites do.</para>
    /// </summary>
    ...
    public async Task PostingALink_AttachesAPreviewAfterTheMessageIsAlreadyDelivered()
    {
        ...
        //
        // This half matters as much as the other. A send must never block on a third-party origin,
        // so "no embeds yet" is the correct and required initial state - clients render the message
        // immediately and let the card arrive later.
        ...
        //
        // Bit 16. Without it a re-unfurl could not tell its own output from a bot's hand-written
        // card, and suppression could not drop one while keeping the other.
        ...
        //
        // The bug this prevents: posting a URL used to mark your own message "(edited)" a second
        // later, by nobody, because the write moved the same timestamp a real edit does.
        ...
        //
        // Functional, not cosmetic: several large sites serve Open Graph tags only to user agents
        // they recognise as link crawlers.
    ...
    public async Task SuppressingAPreview_RemovesItForEveryone()
    {
        ...
        //
        // The unfurl job retries and re-runs on edit, so without a persisted decision a dismissed
        // card comes back on its own. This is the assertion that proves the flag is load-bearing.
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Things that must NOT be unfurled
    // ─────────────────────────────────────────────────────────────────────────

    [Test]
    public async Task AngleBracketedLink_IsNeverFetched()
    {
        // <https://…> is the sender saying "link it, do not unfurl it". Asserted against the origin's
        // own request log rather than against the absence of an embed, because those are different
        // claims: this one is that no request was made at all.

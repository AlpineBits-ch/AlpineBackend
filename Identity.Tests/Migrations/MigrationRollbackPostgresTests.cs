...
/// <summary>Rolls two migrations back over data that the migration itself made possible.</summary>
[TestFixture]
[Category("Postgres")]
public class MigrationRollbackPostgresTests
{
    ...
    /// scoped to <c>(user_id, client_device_id)</c>, so from that point two accounts may hold the
    /// same client-chosen device id - which is routine, because the value is a per-handset string
    /// and handsets get signed into more than one account.
    /// </summary>
    [Test]
    public async Task RollingBackConsolidateDeviceConcepts_WithOneDeviceIdSharedByTwoAccounts_Succeeds()
    {
        ...
        // Neither user lost their device.
    ...
    /// <c>unique</c> over exactly that history.
    /// </summary>
    [Test]
    ...
    /// <summary>Applies or reverts to <paramref name="target"/>.</summary>

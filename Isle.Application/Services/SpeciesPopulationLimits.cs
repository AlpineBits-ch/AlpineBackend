...
/// A cap is the maximum number of that species allowed alive on the server at once; once reached
/// the species is disabled in the spawn menu until a slot frees up. Use <see cref="Unlimited"/>
/// (-1) for species that should never be capped.
/// </summary>
public sealed class SpeciesPopulationLimits
{
    ...
    /// <summary>
    /// Species short name (see <see cref="Species"/>) → max concurrent alive. Case-insensitive.
    /// Override in DI to tune caps without touching the service.
    /// </summary>

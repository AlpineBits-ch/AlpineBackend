...
/// <summary>
/// Covers <see cref="InboxService"/> - the Unread tab.
///
/// <para>The predicate itself is the thing worth pinning: a channel is unread when its denormalized
/// head is newer than the caller's read cursor, or - for a channel they have never opened, which has
/// no read state at all - newer than when they joined the guild. Getting the second case wrong is
/// how an inbox either hides everything a new member has not looked at, or dumps a guild's entire
/// backlog on them the moment they join.</para>
///
...
public class InboxServiceTests(IGuildContextProvider provider)
{
    ...
    /// <summary>A read state outlives the access that produced it. Without a fresh ViewChannel check
    /// the inbox would keep listing - and previewing the contents of - a private channel somebody was
    ...
    /// <summary>A cursor the server did not mint means a stale bookmark, not an attack. Starting
    /// from the top beats a 400 that leaves the tab broken.</summary>
    [Test]

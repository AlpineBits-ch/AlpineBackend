...
/// <summary>
/// Who has held the hill, how long, and who holds it right now.
///
/// <para>Two things are tracked, for two different questions. <c>control</c>/<c>firstseen</c> answer
/// "who should win at the end" — every tick a player has the zone to themselves credits one tick, and
/// standings rank by ticks, earliest-credited breaking a tie (mirrors
/// <see cref="Isle.Api.Services.Quests.QuestProgressLedger"/>'s own tiebreak). <c>holder</c>/<c>since</c>
/// answer "is someone about to win outright" — the current sole occupant and how long they have held it
/// unbroken, reset the moment anyone else steps in or the zone empties. A player can lead on ticks
/// without currently holding the streak that wins early, and that is intentional: contested time counts
/// against nobody's total, but it also cannot extend anyone's held-alone streak.</para>
///
/// <para>Redis-backed for the same reason the quest ledgers are: a match runs up to ten minutes and must
/// survive an API restart, and progress that did not would quietly resolve in favour of whoever the
/// tick loop happened to be crediting when the process came back up.</para>
/// </summary>
        ...
        try
        {
            ...
            return control
                .Select(entry =>
                {
                    ...
                    // A missing arrival time means the HSETNX write was the one that failed. Sorting
                    // them last is the conservative reading — we cannot claim they got there first.

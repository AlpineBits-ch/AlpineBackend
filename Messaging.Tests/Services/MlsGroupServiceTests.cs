...
///
/// <para>The behaviours worth guarding are the ones whose failure is silent and permanent. Toggling
/// mints a <i>new</i> group whose epochs restart at zero, so anything keyed on epoch alone collides
/// across generations and a client decrypts against the wrong keys. Terminating must not destroy the
/// old generation's row either, or the ciphertext still sitting in the context loses the only record
/// of which group can read it.</para>
/// </summary>
[TestFixture]
public class MlsGroupServiceTests
{
    ...
    /// <summary>Fixed clock. Every toggle is cooldown-gated, so tests drive time explicitly rather
    /// than sleeping through a 30 second window.</summary>
    ...
    {
        // A group whose creator had nobody to add sits at epoch 0 until the first commit. Rejecting
        // it would make "enable encryption on an empty channel" fail for no reason.
    ...
    public async Task Enable_OnAChannel_AnnouncesViaTheBus()
    {
        ...
        // Messaging does not know who is in a channel; Guild does. The announcement takes the same
        // route channel messages already take rather than resolving membership twice.
    ...
    public async Task CommitsFromDifferentGenerations_CoexistAtTheSameEpoch()
    {
        ...
        // Both groups have an epoch 1. Keyed on epoch alone the second would collide with the first
        // and a client would decrypt generation 2's traffic against generation 1's keys.

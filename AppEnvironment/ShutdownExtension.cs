...
{
    /// <summary>
    /// Registers a "self" health check that reports Healthy until the app
    /// receives a shutdown signal (SIGTERM), at which point it immediately
    /// reports Unhealthy. Combined with a Kubernetes readinessProbe hitting
    /// /health and a preStop hook that outlives the probe interval, this
    /// ensures the pod is pulled from Service/DNS rotation before it
    /// actually stops accepting connections — avoiding 502s during
    /// rollouts, scale-downs, and manual pod deletion.
    /// </summary>
    ...
    /// <summary>
    /// Hooks ApplicationStopping so the health check starts failing
    /// immediately on shutdown signal. Call this after building the app.
    /// </summary>

...
/// The real values live at that helper's call sites, so it is resolved in a second pass.
/// </summary>
...
/// <summary>
/// Harvests the realtime contract straight out of the source.
///
/// <para><b>Outbound</b> is found by matching invocations bound to
/// <c>Microsoft.AspNetCore.SignalR.ClientProxyExtensions.SendAsync</c>. Matching on the bound symbol
/// rather than on the text <c>SendAsync</c> is what keeps <c>bus.SendAsync</c> (Wolverine),
/// <c>http.SendAsync</c> and the bot gateway's raw <c>_socket.SendAsync</c> out of the results -
/// all three exist in this solution and all three would match a naive text search.</para>
///
/// <para><b>Indirection is real and has to be followed.</b> Four sends pass the event name through
/// as a method parameter. Left unresolved they would silently drop every event that fans out through
/// those helpers, which is not a small tail - so a second pass walks the helpers' call sites.</para>
///
/// <para><b>Inbound</b> is read off the hub type's <c>[HubMethodName]</c> methods, which are
/// already fully typed against the records in <c>RealtimeCommands.cs</c>.</para>
/// </summary>
internal sealed class RealtimeExtractor(SchemaBuilder schemas)
{
    ...
    private void TryOutbound(SemanticModel model, InvocationExpressionSyntax invocation)
    {
        ...
        // Only the *name* decides whether this can be documented here. The payload being a method
        // parameter is irrelevant - its type is resolvable either way, and treating it as
        // indirection dropped every bus handler that pushes its own message straight to the hub.
    ...
    /// <summary>
    /// Walks fan-out helpers back to their call sites, repeatedly.
    ///
    /// <para>A fixpoint loop rather than one pass, because the indirection nests: Social's
    /// <c>PushSideAsync(hub, side, eventName)</c> is called by a method that <em>also</em> takes
    /// <c>eventName</c> as a parameter, so one hop only reaches another parameter. Bounded, so a
    /// mutually recursive pair can never spin.</para>
    /// </summary>

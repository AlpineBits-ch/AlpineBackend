...
/// <summary>
/// Reads Wolverine's own record of what it failed to process.
///
/// <para>Exists because a cross-service message that never arrives is invisible from every other
/// angle: the sender logs nothing (it published successfully), the receiver logs nothing (it never
/// ran), and the assertion just times out. Wolverine writes every message it gave up on to
/// <c>wolverine_dead_letters</c>, with the exception attached - so when a preview never appears,
/// this is the difference between "the handler threw" and "no handler was ever listening", which
/// are the two hypotheses and need opposite fixes.</para>
/// </summary>
internal static class WolverineDiagnostics
{
    ...
    ///
    /// <para>Answers the other half: whether a message was ever <i>delivered</i> to this service at
    /// all. An empty result for a type nobody dead-lettered means nothing was listening.</para>
    /// </summary>

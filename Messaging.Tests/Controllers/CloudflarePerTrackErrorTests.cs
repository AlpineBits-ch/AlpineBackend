...
///
/// <para>Cloudflare reports a failed <em>track</em> inside an HTTP 200: normal
/// <c>sessionDescription</c>, and the failing <c>tracks[]</c> entry carries
/// <c>errorCode</c>/<c>errorDescription</c> in place of a <c>mid</c>. Pulling a track that has not
/// propagated to the publisher's session yet - the race
/// <see cref="CloudflareController"/>'s <c>TracksNewWithRetryAsync</c> was written for - arrives
/// exactly this way, so the retry never runs and the caller gets a 200 it cannot distinguish from
/// a working subscribe.</para>
/// </summary>
[TestFixture]

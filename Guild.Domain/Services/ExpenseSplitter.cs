...
/// <summary>Divides an expense total across participants in integer minor units.
///
/// The invariant every method here upholds: <b>the resulting amounts sum to exactly the expense
/// total</b>. Nothing may be lost or invented to rounding, because these numbers feed member
/// balances, and balances that don't sum to zero make the whole ledger untrustworthy - it's the
/// kind of bug people find in their own bank account rather than in a test.
///
/// Remainders are therefore distributed explicitly (one minor unit each to the first N
/// participants by sorted UserId) rather than left to rounding. Sorting by UserId rather than by
/// input order makes the outcome deterministic and independent of how the client happened to
/// order the request.</summary>
public static class ExpenseSplitter
{
        ...
        long totalMinor, IReadOnlyList<SplitParticipant> participants)
    {
        ...
        // whoever was rounded down hardest. Distributing by biggest fractional part (rather than
        // to the first N) keeps a 1/1/1 weighting identical to an equal split.
        var floored = ordered
            .Select(p =>
            {
...
{
    /// <summary>Greedy largest-creditor / largest-debtor matching. Not provably minimal (that's
    /// NP-hard), but it produces at most n-1 transfers and is the algorithm every splitting app
    /// uses - the point is that four housemates settle up with two payments rather than six.
    ///
    /// <paramref name="balances"/> must sum to zero; the caller builds them from
    /// <see cref="ExpenseSplitter"/> output, which guarantees that.</summary>

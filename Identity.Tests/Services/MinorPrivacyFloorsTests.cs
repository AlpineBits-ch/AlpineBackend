...
///
/// <para>The negative cases are the point. A floor that is only reported and not refused is a
/// suggestion, and a floor that is only refused and not reported is a control that assumes no row has
/// ever held a wider value.</para>
/// </summary>
[TestFixture]
public class MinorPrivacyFloorsTests
{
    ...
    public void Snapshot_ForAMinor_NarrowsEveryFloorWithoutTouchingTheOriginal()
    {
        ...
        Assert.Multiple(() =>
        {
            // This is the assertion that keeps "unlocked on the birthday" true. If the clamp wrote
            // through to the stored row, the day they age out they would find their choices had been
            // quietly overwritten rather than restored.
    ...
    {
        // Messaging resolves the DM policy from this projection, not from the REST endpoint. A floor
        // applied only at the endpoint is a floor every other service routes around.

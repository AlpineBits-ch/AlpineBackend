...
/// <summary>
/// The MLS hardening fixes, each pinned by the failure it prevents.
///
/// <para>Everything here shares one property: the bug it guards against was <b>silent</b>. Nothing
/// threw, nothing logged, and the only symptom was a device that could not read a conversation - so
/// a test is the only thing that can notice a regression.</para>
/// </summary>
[TestFixture]
public class MlsHardeningTests
{
    ...
    public async Task Enable_ForAMissingConversation_LeavesNoGenerationBehind()
    {
        ...
        // Wolverine's transactional middleware commits whatever is tracked when the endpoint
        // returns, so the refusal persisted a live generation for a conversation that does not
        // exist - marking the context encrypted forever with no group behind it.
    ...
    public async Task PublishCommit_RepublishedByTheSameDevice_SucceedsInsteadOfConflicting()
    {
        ...
        // The client merged locally and never saw the response. Answering 409 sends it down the
        // "someone else won, throw yours away" path - discarding a commit that *is* the group's
        // history and forking itself off permanently.
    ...
    public async Task PublishCommit_AddingSomeoneWhoseSecondDeviceGotNoWelcome_NamesThatDevice()
    {
        ...
        // /consume-tokens gave back. A device that had run out of key packages is simply absent from
        // the answer, so it never gets a leaf - and nothing anywhere said so. The person joining
        // reads the conversation on one handset and sees nothing on the other; the person who added
        // them was told it worked.
    ...
    {
        // An update or a removal admits nobody, so there is no coverage question to ask. Comparing
        // against the roster anyway would report every long-standing member's devices as missing on
        // every ordinary commit, and a check that always fires is a check somebody turns off.
        await SeedConversation(AdminId, MemberId);
        var service = ServiceKnowing(
            (MemberId, "device-member-phone"),
            (MemberId, "device-member-desktop"));
    ...
    {
        // Already in the group needs no new Welcome. Without this the second person added to a
        // conversation would drag the first one's devices into the report, which is how a true
    ...
    {
        // Enabling or re-keying mints a group only the welcomed devices will ever hold. A device the
        // enabling client could not fetch a key package for is dropped at that instant and stays
        // dropped - and this path reported nothing at all.
        await SeedConversation(AdminId, MemberId);
        var service = ServiceKnowing(
            (MemberId, "device-member-phone"),
            (MemberId, "device-member-desktop"));
    ...
    public async Task PublishProposal_DoesNotAdvanceTheEpoch()
    {
        ...
        // Processing a proposal advances no client's MLS epoch. The server advancing the group's
        // anyway left every client fetching a proposal at an epoch it could never reach - mobile's
        // catch-up loop counted it as applied and never terminated.
        Assert.That(result.Status, Is.EqualTo(MlsOperationStatus.Ok));
    ...
    public async Task GetCommits_WithNoActiveGeneration_ReturnsOnlyTheMostRecentOne()
    {
        ...
        // Nothing is live now. The filter used to be dropped entirely in this case, so the caller
        // got both generations' commits interleaved by epoch - and a client applying that sequence
        // is feeding one group's commits to another, which is a fork, not a recoverable failure.
        var commits = await _service.GetCommitsAsync(ChannelId, generation: null, sinceEpoch: 0);
    ...
    public async Task PublishChannelCommit_AnnouncesToTheChannelOverTheBus()
    {
        ...
        // published it. Every other member sat on the old epoch and silently stopped being able to
        // read the channel.
    ...
    public async Task AnAutoAdmittedDevice_IsAnnouncedToItsOwnerAndToTheConversation()
    {
        ...
        // Nobody was asked. From the owner's side that is indistinguishable from someone who knows
        // the password adding a device of their own - so being able to notice is the entire
        // compensating control for not being asked.

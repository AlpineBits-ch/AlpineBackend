...
/// <summary>
/// Covers CloudflareController.CreateSession's <c>primary</c> flag.
///
/// CreateSession is also how the caller becomes Connected, so it runs Call.ConnectDevice. That path
/// is destructive when re-entered from a different device: it raises CallDeviceTakeover and nulls
/// the participant's CfSessionId, telling the previous device to tear down. A desktop client that
/// publishes its screen from a separate process opens a second Cloudflare session, and if that
/// session took the connect path it would hang up the very call it is sharing into. These tests pin
/// that boundary.
/// </summary>
[TestFixture]

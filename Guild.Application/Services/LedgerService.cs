...
/// <summary>Balance computation for a ledger channel.
///
/// The invariant: net balances across all members sum to exactly zero. Every expense contributes
/// +total to its payer and -share to each participant, and <see cref="ExpenseSplitter"/> guarantees
/// the shares sum to the total; settlements move whole minor units between two people. So the sum
/// telescopes to zero by construction, and any drift is a bug rather than rounding.</summary>
public class LedgerService(MicroserviceContext ctx)
{
    ...
    /// <summary>Replaces an expense's shares from a client payload. Returns an error string when
    /// the request is invalid, null on success - the caller turns that into a 400.</summary>

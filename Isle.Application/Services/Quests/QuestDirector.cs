...
/// <summary>
/// Chooses what to spawn and where.
///
/// <para>The point of the scoring is population spreading: the further a candidate location is from
/// wherever the crowd currently is, and the emptier it is itself, the better it scores. So when the
/// whole server is parked at one sanctuary, the next quest lands somewhere else and gives players a
/// reason to leave.</para>
/// </summary>
    ...
    ILogger<QuestDirector> logger)
{
    ...
    /// <summary>
    /// Picks the next quest to spawn, or null when nothing should spawn right now. The caller owns
    /// creating and announcing the instance.
    /// </summary>
    ...
    /// <summary>
    /// Higher is better. Emptiness dominates; distance from the hotspot is the tie-breaker that
    /// actively drags players away from a crowded area rather than merely avoiding it.
    /// </summary>
    ...
    /// <summary>
    /// Weighted random over score × template weight. Random rather than always-best so the same quest
    /// does not lock onto the same corner of the map every cycle.
    /// </summary>

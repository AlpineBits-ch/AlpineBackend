...
/// <summary>A label defined on a single Forum/Media channel and applied to that forum's posts
/// (see <see cref="ForumPostTag"/>). Deliberately scoped to one channel rather than to the guild:
/// a tag id is meaningless outside its forum, which is what lets the picker be a plain list and
/// the filter a plain id set, with no cross-forum collisions to disambiguate.
///
/// No navigation property points back at Channel on purpose - ChannelDto is Facet-generated and
/// nested inside GuildDto, so every collection added to the Channel aggregate widens that
/// materialization graph (the crash documented in ThreadEndpoint.GetThreadsAsync). The FK is
/// configured shadow-side in MicroserviceContext instead, which still gives cascade delete.</summary>
public class ForumTag : BaseEntity<ForumTag>, IPrefixedEntity
{
    ...
    /// <summary>A guild custom emoji id. Mutually exclusive with <see cref="EmojiName"/>. Not an
    /// FK - a deleted guild emoji leaves this dangling by design, and the client is told to render
    /// the tag without an emoji rather than a broken image.</summary>
    public string? EmojiId { get; set; }
    ...
    /// <summary>Matches Discord's cap. Enforced at create time only, so an import that somehow
    /// exceeds it stays readable rather than breaking the whole forum.</summary>
    ...
    public class UpdateForumTagParams
    {
        ...
        /// <summary>Empty string clears the emoji; null leaves it untouched. The two-state
        /// distinction is why these aren't just nullable-means-clear.</summary>

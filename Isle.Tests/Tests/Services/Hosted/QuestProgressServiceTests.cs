...
/// <summary>
/// <see cref="QuestProgressService"/>'s guard clauses (stale/never-populated roster; roster unchanged
/// since the last credit) are cheap to exercise directly — they never touch the DI scope at all — via
/// the `internal` <c>SafeTickAsync</c>. The "actually credit presence" branch delegates to
/// <c>QuestCompletionService.TrackPresenceAsync</c>, which already has its own dedicated coverage in
/// QuestCompletionServiceTests, so per the "delegates to already-tested logic" strategy the payload of
/// that call is left to those tests; here we only confirm the dispatch itself is attempted.
/// </summary>
[TestFixture]
public class QuestProgressServiceTests
{
    ...
    public async Task SafeTickAsync_FreshNewRoster_AttemptsATick()
    {
        ...
        // QuestCompletionService isn't registered in this scope, so resolving it throws — caught and
        // logged by SafeTickAsync's own try/catch. What this test cares about is that a scope was opened
        // at all, i.e. that the fresh/new-timestamp guard let the tick through.

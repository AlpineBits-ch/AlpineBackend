...
/// user through Identity must (a) publish UserCreated over the real RabbitMQ broker for Social
/// (a separate real process) to consume and materialize a Profile, and (b) mint a JWT that
/// Social's own JwtBearer middleware - in a third real process - accepts by fetching Identity's
/// real /.well-known/openid-configuration + jwks over real HTTP. Nothing here is mocked or
/// in-process; this is the scenario the harness exists to make possible at all (see the E2E test

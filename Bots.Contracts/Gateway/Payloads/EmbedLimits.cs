...
/// <summary>
/// Discord's documented embed size limits, and the clamping that enforces them.
///
/// <para>These matter far more for unfurled embeds than for bot-authored ones. A bot's embed is
/// written by someone who wanted it to render; an unfurled embed is built from whatever a
/// third-party page put in its <c>&lt;meta&gt;</c> tags, which is attacker-controlled text of
/// attacker-chosen length. Without <see cref="Clamp"/> a hostile page decides how many bytes every
/// message quoting it costs - in Scylla, in Postgres, in the search index, and in every realtime
/// frame that carries the message.</para>
/// </summary>
public static class EmbedLimits
{
    ...
    /// <summary>
    /// Clamps one embed in place to the per-field limits. Truncation is by text element, not by
    /// <c>char</c>: cutting a UTF-16 string at an arbitrary index can split a surrogate pair or a
    /// grapheme cluster and produce a lone half-emoji, which some clients render as a replacement
    /// glyph and at least one JSON parser rejects outright.
    /// </summary>
    ...
    /// <summary>
    /// Takes embeds in order until the combined budget is spent, dropping the rest whole. Dropping
    /// a trailing embed entirely beats truncating every embed proportionally: a card with its
    /// description sliced mid-sentence looks broken, whereas a missing fifth preview looks like a
    /// missing fifth preview.
    /// </summary>
...
public static class EmbedColor
{
    ...
    /// <summary>Parses <c>#rrggbb</c>, <c>rrggbb</c> or a decimal integer. Returns null on anything
    /// else rather than throwing - the input is a third-party page's <c>theme-color</c>.</summary>

...
public class MlsJoinRequestServiceTests
{
    ...
    /// <param name="actors">Distinct users the server has seen act on the group.</param>
    ...
    public async Task Submit_RecordsTheHashOfTheExactKeyPackage()
    {
        ...
        // The hash is what the committing client checks before adding.
        var stored = await _context.MlsJoinRequests.SingleAsync();
    ...
    /// <summary>The one thing that stays forbidden.</summary>
    [Test]
    ...
    /// <summary>The device id is compared exactly.</summary>
    [Test]
    ...
    /// <summary>Own-device approval is not a way around the peer threshold.</summary>
    [Test]

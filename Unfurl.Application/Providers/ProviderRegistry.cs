...
///
/// <para><b>This class is the only thing in the service that may populate
/// <see cref="EmbedPayload.Video"/>, and it may only do so from the patterns below.</b> A client
/// renders <c>video.url</c> inside an iframe; if generic Open Graph parsing could reach that field,
/// any page on the internet could hand every viewer of a chat message an iframe of its choosing.
/// The generic path is structurally incapable of it - <c>PageMetadata</c> has no field that could
/// carry one - and <c>ProviderWhitelistTests</c> asserts that stays true.</para>
///
/// <para>Each player URL is <i>constructed</i> from a video id we extracted, never taken from the
/// page. So even a compromised provider page cannot choose the iframe src: the worst it can do is
/// point us at a different video on the same provider.</para>
/// </summary>
public static class ProviderRegistry
{
    ...
    /// <summary>Attempts to match a URL to a known player. Returns null for everything else, which
    /// is the overwhelming majority of links and falls through to generic parsing.</summary>

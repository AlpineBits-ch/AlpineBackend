...
/// <summary>
/// King of the Hill's clock. One loop rather than quest's two: unlike quests — which have independent
/// "spawn new" and "credit presence" concerns running at different cadences against independent rows —
/// KOTH only ever has one thing that can be true at a time (idle/cooldown, or one match running), and
/// both branches gate on the same 30s <see cref="WorldRosterCache"/> refresh anyway.
///
/// <para>There is no separate "resume after restart" step: <see cref="KingOfTheHillMatchStateStore"/> is
/// read fresh at the top of every tick, so the first tick after a restart finding a marker there
/// <i>is</i> the resume — <see cref="GameModeInstance.Resume"/> reconstructs the same match a normal tick
/// would already be ticking, no special-casing required.</para>
/// </summary>
    ...
    ILogger<KingOfTheHillDirectorService> logger) : BackgroundService
{
    ...
    internal async Task SafeTickAsync(CancellationToken ct)
    {
        try
        {
            ...
            {
                // Silent before this: a live match just sat un-credited for the cycle with nothing in
                // the log to explain why the standings came up empty later. Only worth a line when a
                // match is actually running — a stale roster while idle is unremarkable.

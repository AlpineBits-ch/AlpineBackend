...
/// <summary>Shared expenses on a <see cref="ChannelType.Ledger"/> channel.
///
/// Every amount crossing this boundary is in <b>minor units</b> (rappen, cents) as a whole number.
/// There is no decimal anywhere in the money path: the split maths in
/// <see cref="ExpenseSplitter"/> depends on integer arithmetic to guarantee that shares sum to the
/// total and balances sum to zero. Clients format for display; they never send 12.34.</summary>
[Authorize]
public class LedgerEndpoint
{
    ...
    /// <summary>The minimal-ish set of payments that clears the house. Four flatmates settle up
    /// with two transfers instead of six.</summary>
        ...
        [NotBody] ClaimsPrincipal user)
    {
        ...
        // Switching currency on a ledger with history is a foot-gun the client should warn about.

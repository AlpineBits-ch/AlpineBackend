...
/// <summary>
/// Adding a recovery code to an account that already has a master key.
///
/// <para><b>This is the only path that matters for the accounts that already exist.</b> Every one of
/// them was set up before the master key was wrapped twice, so every one of them has a password
/// wrapping and nothing else - which is exactly the state a password reset destroys. The retrofit
/// has to work, and it has to work without disturbing the blobs those accounts have already
/// stored.</para>
///
/// <para>The two ways of getting it wrong are both silent, which is why these are pinned. Returning
/// <c>200 OK</c> without writing the wrapping shows the user a recovery code that opens nothing while
/// telling them they are protected. Bumping the version to force the write orphans every backup blob
/// on the account - for a master key that did not change.</para>
/// </summary>
[TestFixture]
public class RecoveryKeyRetrofitTests
{
    ...
    public async Task AddingARecoveryCodeAtTheSameVersion_PersistsTheWrappingAndOrphansNothing()
    {
        ...
        Assert.Multiple(() =>
        {
            // The wrapping actually landed. Returning Ok without writing it is the failure that
            // shows a user a recovery code which opens nothing.
    ...
    public async Task AddingARecoveryCode_LeavesExistingBackupBlobsReadable()
    {
        ...
        // An additive write must not move the envelope past it.

...
public class PasswordResetCodeServiceTests
{
    ...
    // ── Entropy and failed-attempt lifecycle ───────────────────────────────── The code used to be
    // the first 6 hex chars of a GUID (2^24) and a wrong guess left it intact for its full
    // 15-minute life, so a single live code could be guessed without limit.

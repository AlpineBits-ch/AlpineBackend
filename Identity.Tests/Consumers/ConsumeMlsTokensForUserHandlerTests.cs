...
///
/// <para>A well-formed KeyPackage header is required - <see cref="UserKeyPackage.Create"/> derives
/// the ciphersuite from bytes 2-3, so the fixtures carry a real MLS10 + ciphersuite-1 prefix rather
/// than arbitrary bytes.</para>
/// </summary>
[TestFixture]
public class ConsumeMlsTokensForUserHandlerTests
{
    ...
    public async Task Handle_UserWithTwoDevices_ReturnsOneTokenPerDevice()
    {
        ...
        // token, so two devices produced duplicates and the later Single(...) lookup threw. This is
        // the ordinary case for anyone running desktop plus phone.
    ...
    public async Task Handle_TwoDevicesOneOutOfKeyPackages_AnswersWithBothAToken_AndAnUnreachable()
    {
        ...
        // packages. The answer must not be "user-1 is reachable" - that is true of the user and
        // false of the desktop, and a caller that reduces this to a per-user check builds a group
        // the desktop can never read. Both halves have to be in the response for the caller to have
        // any chance of telling a human.
        var phone = SeedDevice("user-1", "device-phone");

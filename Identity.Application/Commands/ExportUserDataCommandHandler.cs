...
/// <summary>
/// Identity's participant in the <c>ExportUserDataSaga</c> fan-out (T1-7) - the read-side sibling of
/// <see cref="PurgeUserDataCommandHandler"/>.
///
/// <para><b>Everything here is the subject's own row.</b> Every table Identity owns is keyed by
/// <c>UserId</c>, so this fragment is scoped by construction; there is no join that could reach
/// another account, and the test for this handler asserts that a second user's address does not
/// appear in the output.</para>
///
/// <para><b>Credentials are excluded on purpose, and their absence is not a gap in the response.</b>
/// The password hash, the security stamp, both wrappings of the master key, device identity keys and
/// key packages, backup blobs and the literal push-token strings are all deliberately left out. None
/// of them is information <i>about</i> the subject in any sense they can use - a password hash tells
/// its owner nothing - and every one is a live credential. Art. 15 is a right to one's personal data,
/// not an obligation to assemble the account's key material into a single portable file that will
/// then be emailed, synced and left in a downloads folder. Push tokens are reported by transport and
/// registration date so the subject can see <i>that</i> a device is registered for notifications
/// without the token itself.</para>
///
/// <para><b>The status flip lives here rather than in a message of its own.</b> "Running" means the
/// fan-out reached a participant, and Identity is the first participant that can say so against the
/// row. Idempotent - see <see cref="DataExportRequest.BeginRunning"/>, which will not move a request
/// that already failed or already expired. No <c>SaveChangesAsync</c>: Wolverine's transactional
/// middleware commits the context, per this repo's convention.</para>
/// </summary>
        ...
        ILogger<ExportUserDataCommandHandler> logger)
    {
        ...
        var fragment = new
        {
            account = new
            {
                ...
                // Client-owned UI state with no server semantics (T0-6). Included because the user
                // put it there and it is theirs.
            ...
            preferences = preferences is null ? null : new
            {
                ...
                // The legacy privacy flags, still readable off GET /users/self. Reported alongside
                // the record that superseded them so the subject sees what is actually stored.

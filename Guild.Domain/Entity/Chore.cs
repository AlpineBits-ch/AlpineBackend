...
/// <summary>A recurring household task. Rotation reuses an existing <see cref="Aggregates.Role"/>
/// as the pool ("Flatmates"), so adding someone to the rota is just giving them the role and the
/// move-out flow removes them from it for free.
///
...
public class Chore : BaseEntity<Chore>, IPrefixedEntity
{
    ...
    /// <summary>Plain interval rather than an RRULE. A house rota is "every N days from this
    /// date"; full calendar recurrence buys complexity nobody asked for.</summary>
    public int IntervalDays { get; set; } = 7;

    /// <summary>The first due date. Later occurrences are anchor + n*interval, so a chore
    /// completed early doesn't drag the whole schedule forward.</summary>
    public DateTimeOffset AnchorAt { get; set; }

    /// <summary>The fairness weight: roughly how long this takes. Rotation assigns whoever has
    /// the fewest accumulated minutes, so "takes the bins out" and "cleans the bathroom" don't
    /// count the same. Snapshotted onto each occurrence at generation time.</summary>
    public int EffortMinutes { get; set; } = 15;
    ...
    /// <summary>When the next occurrence is due. Kept denormalized so the reconcile sweep can
    /// find chores needing generation with an index scan rather than recomputing every rota.</summary>
    ...
    public DateTimeOffset AdvanceFrom(DateTimeOffset after)
    {
        ...
        // Bounded rather than while(true): a chore paused for years shouldn't spin here. 4096
        // steps is ~11 years of daily cadence, far past any real rota.

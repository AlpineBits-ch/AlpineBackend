...
///
/// <para><b>Why this exists.</b> 267 of the ~278 endpoints in this solution return
/// <c>Task&lt;IResult&gt;</c> and there is not a single <c>Produces&lt;T&gt;</c> or
/// <c>[ProducesResponseType]</c> anywhere, so ASP.NET's OpenAPI generator can infer nothing: it emits
/// paths, parameters and auth with completely empty responses. The alternative to this analyser is
/// migrating 267 signatures to <c>Results&lt;Ok&lt;T&gt;, NotFound&gt;</c> by hand.</para>
///
/// <para>Every <c>return Results.X(...)</c> in the method body is a statement of intent that is
/// already in the code - <c>Results.Ok(page)</c> names the success type, <c>Results.NotFound()</c>
/// names a status. Reading them back out is exact for the status codes and as good as the source for
/// the body type. The result is emitted as an overlay the gateway merges into the aggregated
/// document, so no endpoint has to change.</para>
///
/// <para><b>Limits, stated rather than hidden.</b> A helper that builds the <c>IResult</c> elsewhere
/// and returns it is not followed, and a body type that is only known at runtime cannot be resolved.
/// Both are counted and reported.</para>
/// </summary>
internal sealed class EndpointExtractor(SchemaBuilder schemas)
{
    ...
    private void TryEndpoint(SemanticModel model, MethodDeclarationSyntax method, string project)
    {
        ...
        if (responses.Count == 0)
        {
            ...
            // overlay - ASP.NET already infers its response schema from the signature. Reporting
            // those as gaps would bury the ones that are actually undocumented.
    ...
    ///
    /// <para>The delegation hop is load-bearing: several endpoints are one-line forwarders to a
    /// shared private helper (<c>PermissionOverwriteEndpoint</c> has eight of them). Without it those
    /// endpoints look like they return nothing at all.</para>
    /// </summary>

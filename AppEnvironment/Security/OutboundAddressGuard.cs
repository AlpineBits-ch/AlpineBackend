...
/// <summary>
/// Refuses to open sockets to addresses that are not on the public internet.
///
/// <para>This is the shared core of every SSRF defence in the product. Two callers with very
/// different threat models use it: federation, where an operator supplies a peer's hostname, and
/// the unfurler, where <b>anyone who can type a URL into a chat box</b> supplies it. The second is
/// why this lives here rather than in either service - one implementation, reviewed once, and a fix
/// lands everywhere.</para>
///
/// <para>Validation happens at <see cref="SocketsHttpHandler.ConnectCallback"/> time, against the
/// IP actually being dialled, rather than only up-front against the hostname. That is what makes
/// DNS rebinding - a name that resolves to a public address on the pre-flight check and a private
/// one on the real connect - ineffective. Redirects have to be handled separately by the caller,
/// because a public host can otherwise bounce a request inward.</para>
/// </summary>
public static class OutboundAddressGuard
{
    /// <summary>
    /// A connect callback that dials only routable addresses.
    /// </summary>
    /// <param name="allowPrivateTargets">
    /// True outside Production, so a self-hoster, a developer, or the E2E harness can point these
    /// clients at localhost. Never true in Production.
    /// </param>
    ...
    public static bool IsBlocked(IPAddress address)
    {
        ...
        if (address.AddressFamily == AddressFamily.InterNetworkV6)
        {
            ...
            // IPv4-mapped (::ffff:a.b.c.d) - unwrap so the IPv4 rules below still apply. Without
            // this, ::ffff:169.254.169.254 walks straight past every IPv4 rule.
    ...
    ///
    /// <para>The literal check here is for a clear early rejection only. A <i>hostname</i> that
    /// resolves to a private address is caught by <see cref="CreateConnectCallback"/>, which is the
    /// check that actually matters.</para>
    /// </summary>

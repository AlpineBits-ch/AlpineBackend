...
public class GetUserMentionsHandler
{
        ...
        GetUserMentionsRequest request, IMentionIndexRepository repo)
    {
        ...
        var rows = await repo.GetPageAsync(new MentionPageQuery
        {
            ...
            // Bounded by the repository's own clamp, so this cannot become an unbounded scan.
        ...
        DeleteUserMentionRequest request, IMentionIndexRepository repo)
    {
        ...
        // Reported as deleted whether or not a row was there. Dismissing twice is the same outcome
        // as dismissing once, and the caller has nothing useful to do with the difference.

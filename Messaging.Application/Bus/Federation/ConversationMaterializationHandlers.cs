...
public class ConversationMaterializationHandlers
{
    public static async Task Handle(FederatedConversationCreatedReceived message, MicroserviceContext db, CancellationToken ct)
    {
        ...
        db.Conversations.Add(new Conversation
        {
            ...
            // available here to populate a real cached display name, so we default to the federated
            // user id (same empty-key convention as a non-MLS local member) rather than leaving
            // them null, which would fail this SaveChangesAsync with a constraint violation.

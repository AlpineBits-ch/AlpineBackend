...
public class MessageUpdatedHandler
{
        ...
        MicroserviceContext ctx, IMessageBus bus)
    {
        ...
        {
            // The author is excluded from their own edits - their client already rendered the change
            // it just made. But an update the author did not cause (a link preview being attached,
            // or suppressed by a moderator) is news to them too, and leaving them out would mean the
            // person who posted the link is the only one who never sees its preview appear.

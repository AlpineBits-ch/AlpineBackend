...
/// <summary>
/// Covers <see cref="GuildTypingHandler"/> under privacy spec T2-18.
///
/// <para>The setting is enforced at the emit site, not the render site, so the assertions are about
/// who the hub was asked to send to - a recipient removed here never had the fact on their socket at
/// all. The reciprocity case is the one that carries the design: a user who does not send typing
/// indicators does not receive them, because anything else lets one person take without giving and
/// the setting stops being usable.</para>
/// </summary>
[TestFixture]

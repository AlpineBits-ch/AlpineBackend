...
/// <summary>
/// Covers the social.* websocket fan-out. The load-bearing property in every test is that both
/// sides get the event with *their own* row id and status - a payload that leaked the actor's row
/// id to the counterpart would let the client POST /accept on a relationship it doesn't own.
/// </summary>
[TestFixture]

...
/// same cursor-page question, and required to answer it identically - <b>against a real Scylla node
/// and a real Postgres database</b>.
///
/// <para><b>Why this fixture exists.</b> The two backends reach a page by entirely different routes.
/// Scylla resolves the anchor's millisecond by hand and slices on one clustering column, because its
/// table is clustered <c>(created_at DESC, message_id ASC)</c> and no CQL relation matches that
/// ordering (see <c>ScyllaMessageRepository.FetchRelativeAsync</c>). Postgres expresses the same
/// cursor as a longhand <c>(created_at, id)</c> comparison in one <c>WHERE</c>. Two implementations
/// that agree on the easy cases and part ways on a same-millisecond tie is not a hypothetical: the
/// tie-break is the exact place the Scylla side was broken, and the Postgres side resolves it with a
/// <b>collation</b> while the Scylla side resolves it with a byte comparison. <b>A client that pages
/// a conversation on one deployment and the same conversation on another must not be shown a
/// different history.</b></para>
///
/// <para><b>Neither backend may be faked here and neither may be InMemory.</b> A fake mapper has no
/// clustering order, and the EF InMemory provider compares strings with .NET's culture rules rather
/// than with the collation Postgres will actually use - so the one question this fixture exists to
/// answer, whether the tie-break agrees, is precisely the question those two cannot be asked.</para>
///
/// <para>Skipped, loudly and by name, unless <b>both</b> <c>ECHO_TEST_SCYLLA</c> (<c>host:port</c>)
/// and <c>ECHO_TEST_POSTGRES</c> (an ordinary connection string to any reachable database) are set,
/// so CI without the containers still passes.</para>
/// </summary>
[TestFixture]
[Category("Scylla")]
[Category("Postgres")]
public class MessageCursorPagingBackendParityTests
{
    ...
    /// <summary>The production model against a throwaway database - the real column types and the
    /// real collations, not a rebuilt approximation of them. The enum mappings and the snake-case
    /// convention have to be restated because <c>OnConfiguring</c> is overridden (otherwise it would
    /// read the environment's connection string and point this fixture at a development database),
    /// and without them the model's snake-cased index filters do not match the generated
    /// PascalCase columns and <c>EnsureCreated</c> fails outright. Same shape as
    /// <c>MlsConcurrentCommitPostgresTests.NewContext</c>.</summary>
    ...
    /// <summary>Writes the <b>same</b> row to both stores - same id, same timestamp, same partition.
    /// Two Message instances rather than one shared object: EF tracks and mutates what it is handed,
    /// and a poco the Cassandra mapper has also seen is a poco whose state depends on the order the
    /// two writes happened in.</summary>
    ...
    {
        // A page cap inside a same-millisecond group: Scylla decides which members are "nearest" with
        // an ordinal comparison in process, Postgres with ORDER BY on a collated text column. If the
        // two collations disagreed about any pair of message ids, this is where it would surface -
        // and it would surface as two clients seeing different messages.
    ...
    public async Task ScrollingEitherBackendToExhaustionVisitsTheSameMessagesInTheSameOrder()
    {
        ...
        // twice or miss one. Page size 2 across a partition with two multi-member millisecond groups
        // puts a boundary inside a group repeatedly, in both directions.
        var contextId = NewContextId();
    ...
    {
        // The deprecated (take, skip) overloads. They drift under concurrent writes on either
        // backend - that is inherent to offset paging and is why cursor paging exists - but on a
        // partition nobody is writing to they must at least agree with each other, and neither may
        // lose a row to a page boundary landing inside a millisecond.
        var contextId = NewContextId();
    ...
    public async Task PostgresOrdersMessageIdsTheSameWayScyllasClusteringKeyDoes()
    {
        ...
        // rather than as a mystery off-by-one in someone's scrollback. Message ids are `mesg_` plus a
        // 26-character Crockford base32 ULID body (Ids.Identifier) - digits and uppercase letters
        // only, and a prefix identical on every row - which is the property that makes a collated
        // ORDER BY and a byte comparison agree.
        var contextId = NewContextId();

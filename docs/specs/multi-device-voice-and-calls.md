...
This document describes the contract as built.

> The device id is validated against the caller's registered devices; push tokens, login sessions and
> device registration are all consolidated around it.

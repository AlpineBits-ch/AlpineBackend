...
/// <summary>
/// Social's participant in the export fan-out (T1-7).
///
/// <para><b>The negative test is the reason this file exists.</b> Relationships are the one place in
/// this service where another person's profile - username, bio, accent colour - is a single join
/// away, and joining is exactly what an export handler must not do. A counterparty is an opaque
/// profile id and nothing else.</para>
/// </summary>
[TestFixture]
public class ExportUserDataCommandHandlerTests
{
    ...
    public async Task Handle_DoesNotDiscloseBlocksPlacedAgainstTheSubject()
    {
        ...
        // T0-3: a block must not be visible to the person blocked. Exporting the blocker's row into
        // the subject's archive would route straight around that.

...
/// merging them.
///
/// <para><b>Why at runtime rather than at build time.</b> The build-time generator
/// (<c>Microsoft.Extensions.ApiDescription.Server</c>) boots the host to extract the document, and
/// this gateway opens a Redis connection at startup - so generating docs would mean standing up
/// infrastructure in CI. Fetching from the running services also guarantees the docs describe what
/// is actually deployed rather than what was last built.</para>
///
/// <para>Cached and refreshed in the background: a docs page must not fan out eight HTTP calls per
/// view, and a service being briefly down should serve slightly stale docs rather than a 502.</para>
/// </summary>
    ...
    ILogger<OpenApiAggregator> logger)
{
    ...
    public async Task<string> GetDocumentAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Only replace a good document with another good one. If every service is unreachable
            // during a rolling restart, stale docs beat an empty page.

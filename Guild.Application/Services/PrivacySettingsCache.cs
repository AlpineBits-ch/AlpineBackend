...
///
/// <para><b>Redis, not in-memory.</b> Guild runs several instances and these are read on every
/// presence broadcast, every typing indicator and every channel push. A stale per-pod copy means a
/// user's "hide my push content" toggle silently does not apply on whichever pod happened to handle
/// their message - which is exactly the failure mode a privacy control cannot have. The entry is
/// dropped by <c>PrivacyCacheInvalidationHandler</c> when
/// <c>UserPrivacySettingsChangedEvent</c> arrives, and expires on its own after
/// <see cref="Ttl"/> so a missed event self-heals rather than persisting forever.</para>
///
/// <para><b>Fail closed.</b> If neither the cache nor the bus can answer for a user, the caller gets
/// <see cref="Restrictive"/> - never a permissive default. The operational consequence is real and
/// deliberate: while Identity is unreachable, guild pushes carry no content and typing indicators
/// stop flowing. That is the direction a privacy control must fail in. The fallback is <b>not</b>
/// written to the cache, so an outage cannot poison it for the following five minutes.</para>
/// </summary>
    ...
    ILogger<PrivacySettingsCache> logger)
{
    ...
    /// <summary>
    /// What an unresolvable user is treated as. Every field is the most restrictive value the enum
    /// or the flag admits - <c>Friends</c> for DMs (the spec's named floor), <c>Nobody</c> for
    /// everything else, sharing off, receipts off, push content hidden.
    /// </summary>
    ...
    /// <summary>
    /// Resolves many users at once: one Redis read per user, then <b>one</b> bus round trip for
    /// whatever was missing. Batched because every caller here is a fan-out - the viewers of a
    /// channel, the recipients of a push - and the per-user shape would put a round trip on each.
    /// </summary>
    ...
    /// <summary>One user. Convenience over <see cref="GetAsync(IEnumerable{string}, CancellationToken)"/>;
    ...
    /// <summary>Drops one user's entry. Called from the change-event handler; safe to call for a
    /// user with no entry.</summary>

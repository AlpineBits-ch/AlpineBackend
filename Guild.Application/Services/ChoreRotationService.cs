...
/// <summary>Decides who gets the next chore, and reports how the workload is actually distributed.
///
/// Assignment is <b>not</b> round-robin. The next occurrence goes to whoever in the rotation pool
/// has done the fewest weighted minutes over the balance window. That difference is the whole
/// point of the module: a plain rota silently rewards whoever skips, because their turn just comes
/// round again next week with no memory of the miss. Weighting by
/// <see cref="Chore.EffortMinutes"/> also stops "take the bins out" counting the same as "clean
...
public class ChoreRotationService(MicroserviceContext ctx)
{
    ...
    /// <summary>Who should take the next occurrence. Lightest-loaded first; ties broken by who
    /// went longest without being assigned, then by user id so the result is deterministic.</summary>
    ...
    /// <summary>Creates the occurrence for a chore's current NextDueAt and advances the schedule.
    /// Staged only - the caller commits.
    ///
    /// Idempotent by the unique (ChoreId, DueAt) index: both the scheduled message and the
    /// reconcile sweep may fire for the same slot, and exactly one of them must win.</summary>

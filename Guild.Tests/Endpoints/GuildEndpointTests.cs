...
public class GuildEndpointTests
{
    ...
    // Discord's default_message_notifications: what a member who has set nothing falls back to.
    // Also the switch that bounds per-message notification work in a large guild, so a silent
    // failure to persist it is a performance regression as well as a settings one.

...
public class Call : Aggregate<Call>, IPrefixedEntity
{
    ...
    /// <summary>Marks a participant's media session as connected from a specific device. Used by
    /// both <see cref="Accept"/> (the callee's ring-answer flow) and the Cloudflare
    /// session-creation path, which is how the *caller* becomes connected - they never "accept"
    /// their own call. Detects a device takeover (this user already connected from a different
    /// device) the same way regardless of which path triggered it.</summary>
    ...
    /// <summary>Removes just this participant from a still-active call. If other participants
    /// remain connected, the call keeps running. If this drops the call to exactly one connected
    /// participant, that participant gets a grace period (see <see cref="CallWentAlone"/>) before
    /// the call is auto-ended, rather than ending immediately - covers both group calls and 1:1
    /// calls (a 1:1 hangup no longer instantly kills the call). No-ops if this isn't the
    /// participant's active device (a stale/duplicate leave from a device that already lost the
    /// connection, e.g. via takeover).</summary>

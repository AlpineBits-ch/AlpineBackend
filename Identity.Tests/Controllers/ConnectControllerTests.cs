...
/// <summary>
/// Covers the OpenIddict token endpoint (POST /connect/token): password grant (incl. MFA
/// required/invalid/valid-TOTP/recovery-code branches and the not-allowed-to-sign-in check),
/// refresh-token grant, the custom Steam grant, and client-credentials (bot) grant.
/// </summary>
[TestFixture]
public class ConnectControllerTests
{
    ...
    /// to call `Forbid("message")`, which ASP.NET Core's ControllerBase.Forbid(params string[]
    /// authenticationSchemes) interprets as an authentication *scheme name*, not a display message.
    /// Since no scheme named "User is not allowed to sign in" is registered, that threw an
    /// InvalidOperationException instead of returning a clean 403 - now fixed to
    /// StatusCode(403, message).</summary>
    [Test]

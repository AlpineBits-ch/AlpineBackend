...
public class GuildOnboardingPrompt : BaseEntity<GuildOnboardingPrompt>, IPrefixedEntity
{
    ...
    /// <summary>At most one option may be picked. Enforced when a member answers, not by the
    /// schema.</summary>
    public bool SingleSelect { get; set; }

    /// <summary>The member cannot finish onboarding without answering. Only meaningful together
    /// with <see cref="InOnboarding"/> - a required prompt that never appears in the join flow
    /// would be unanswerable.</summary>
...
/// <summary>One answer a member can pick. Carries the actual effect of onboarding: the roles it
/// grants and the channels it makes visible.</summary>
public class GuildOnboardingPromptOption : BaseEntity<GuildOnboardingPromptOption>, IPrefixedEntity
{
    ...
    /// <summary>Roles granted when this option is picked. Validated at config time AND re-checked
    /// at grant time (a role can gain privileged permissions after the prompt was written) - see
    /// OnboardingValidationService.</summary>

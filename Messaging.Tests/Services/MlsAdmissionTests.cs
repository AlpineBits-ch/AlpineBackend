...
///
/// <para>The thing being guarded throughout is that <b>the server is not trusted</b>. It carries a
/// nonce and a signature between two of the user's devices and enforces the two rules it can
/// legitimately enforce - one answer per nonce, inside its window. It never validates a proof,
/// because it cannot: the key is derived from the account master key, which it holds only wrapped.
/// A test that asserted the server rejected a bad proof would be asserting a capability it must not
/// have.</para>
/// </summary>
[TestFixture]
public class MlsAdmissionTests
{
    ...
    public async Task Threshold_ForAConversation_IsAlwaysOne()
    {
        ...
        // A DM has two humans in it. Requiring two approvals means the one person who is not the
        // requester cannot admit them alone, and the conversation deadlocks with no way out.
    ...
    public async Task Threshold_TreatsAnEmptyActivatedByAsNoActor()
    {
        ...
        // generation. Counting the empty string as a real person made a one-member group demand two
        // approvals it could never collect - permanently unable to admit anyone.
    ...
    public async Task Submit_AfterADeviceWasAlreadyAutoAdmittedToday_FallsBackToManual()
    {
        ...
        // A burst of admissions is what a compromise looks like, so the second one costs a tap. It
        // must not *fail* - a legitimate user with two new devices still has to get both in.
    ...
    public async Task Submit_ForTheSameDeviceInASecondConversation_KeepsItsBudget()
    {
        ...
        // The budget counts devices, not requests. One handset joining five conversations is one
        // admission; charging it five would make an ordinary restore fall back to manual for no
        // reason at all.
    ...
    public async Task Proof_IsRelayedVerbatimAndNeverValidated()
    {
        ...
        // Deliberately nonsense. The server cannot tell - it does not hold the account master key -
        // and must not behave as though it could.

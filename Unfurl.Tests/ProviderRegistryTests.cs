...
/// <summary>
/// The provider registry and the whitelist rule it exists to enforce.
/// </summary>
public class ProviderRegistryTests
{
    ...
    /// <summary>
    /// A client renders <c>video.url</c> in an iframe. If anything derived from a fetched page could
    /// reach that field, any site on the internet could put an iframe of its choosing in front of
    /// every person who saw a link to it. The defence is structural rather than a check: the type
    /// the generic parser returns has no field that could carry a player, so there is no code path
    /// to review. This test is what keeps that true.
    /// </summary>
    [Test]

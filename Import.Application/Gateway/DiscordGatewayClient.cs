...
/// Echo-owned import bot's token - the reverse of Bots.Application/Gateway/GatewayWebSocketMiddleware.cs
/// (that accepts inbound connections FROM Discord bot libraries; here Echo itself is the client
/// speaking to real discord.com). One connection covers every linked guild the bot is in -
/// Discord dispatches guild_id-tagged events over the same socket regardless of guild count.
/// GUILDS intent only (1 &lt;&lt; 0) - no privileged intents, since sync never touches members or
/// messages. Reuses Bots.Contracts' envelope/op-code/Hello payload shapes - same wire protocol,
/// just the opposite connection direction.
/// </summary>
        ...
        while (!ct.IsCancellationRequested && socket.State == WebSocketState.Open)
        {
            ...
            switch ((GatewayOpCode)envelope.Op)
            {
                ...
                case GatewayOpCode.InvalidSession:
                    ...
                    // Clearing _sessionId makes the next connection attempt IDENTIFY fresh;
                    // the periodic reconciliation BackgroundService is the safety net that
                    // corrects any drift from events missed during the gap.

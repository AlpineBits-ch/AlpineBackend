...
namespace Identity.Infrastructure.Migrations
{
    ...
    /// tombstone needs.
    ///
    /// <para><b>Purely additive.</b> Three new tables and one nullable column; nothing existing is
    /// altered, narrowed or dropped, and no database-level enum type is touched - the four new enums
    /// are string columns precisely so this migration cannot collide with a concurrently-authored one
    /// over the shared <c>AlterDatabase</c> annotation block.</para>
    ///
    /// <para><b>The rollback is lossy, in one specific way worth saying out loud:</b> dropping
    /// <c>user_consents</c> destroys every record that an account ever accepted anything, and there is
    /// nowhere else to put them - no pre-existing column can express "user X accepted Terms v2 at
    /// time T from address A". Re-running the Up afterwards produces an instance where every account
    /// owes a fresh consent. The other two tables and the column are ordinary losses.</para>
    /// </summary>

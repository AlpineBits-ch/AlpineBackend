...
[TestFixture]
public class UserActiveHandlerTests
{
    ...
    [Test]
    public async Task Handle_ExistingProfile_SetsOnlineAndUpdatesLastSeen()
    {
        ...
        await UserActiveHandler.Handle(new UserActiveEvent { UserId = "user-1" }, _context);

        var stored = _context.Profiles.Single(p => p.UserId == "user-1");
        Assert.Multiple(() =>
        {
            Assert.That(stored.OnlineStatus, Is.EqualTo(OnlineStatus.Online));
    ...
    public void Handle_UnknownUser_DoesNotThrow()
    {
        Assert.DoesNotThrowAsync(() =>
            UserActiveHandler.Handle(new UserActiveEvent { UserId = "no-such-user" }, _context));
    }
}

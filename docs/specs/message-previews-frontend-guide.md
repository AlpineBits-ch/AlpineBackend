...
> **Corrected:** the multi-word fields above were previously documented camel-cased
> (`proxyUrl`, `iconUrl`, `contentType`, `placeholderVersion`). They are **snake_case on the wire**,
> as Discord's own embed object has them. `embedsJson` is an opaque string produced by a different
> serializer from the message fields around it, so `message.editedAt` and `embed.proxy_url` are both
> correct in the same payload. `Echo.E2E.Tests/Scenarios/LinkPreviewFlowTests.cs` asserts the wire
> names directly.
...
**Always render `proxy_url` when it is present.** `url` is the third-party origin's own address, and
loading it directly hands that origin the IP address, rough location and read time of every person
who scrolls past the message - from a link one other person posted. `proxy_url` is our re-hosted
copy and leaks nothing.

```ts
const src = media.proxy_url ?? media.url;   // fall back only if the proxy is absent
```

`proxy_url` arrives as a **fully-qualified absolute URL** - `https://api.venta.gg/api/v1/previews/media/{hash}`
...
`placeholder_version` is `1` for BlurHash. **Check it** - if you see a version you do not recognise,
...
- [ ] `proxy_url` preferred over `url` for every image.
...
## 13. Links to this instance

A link to *this instance* - an invite, a wiki page - is not fetched at all. It is recognised by host
and resolved in-process, and it produces a `venta.*` embed with its own shape and its own refresh
rules. That is a separate document:
[`../../Messaging.Application/docs/embeds-frontend-guide.md`](../../Messaging.Application/docs/embeds-frontend-guide.md).

Read it before shipping: it is also where both clients are told which of their existing invite and
wiki link handling to delete.

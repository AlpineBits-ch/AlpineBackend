...
///
/// <para>An export that hangs at <c>Running</c> is the failure being fixed: the subject watches a
/// spinner, the 24-hour rate limit stops them asking again, and the statutory month expires. Unlike
/// a half-finished erasure a half-finished disclosure can be honest about itself, so the deadline
/// assembles what it has with an explicit error section for each service that never answered.</para>
/// </summary>
[TestFixture]

...
/// <summary>
/// Keeps <see cref="WorldRosterCache"/> fresh. One RCON <c>GetPlayerData</c> per tick returns the
/// whole roster — name, species, growth and position for everyone — which is the only source in this
/// service that can answer "where is the population right now".
///
/// <para>This runs its own tick rather than sharing <c>PopulationLimitService</c>'s: the two have
/// different cadences and failure tolerances, and species-cap enforcement is not something a quest
/// feature should be able to break. Both go through <see cref="IRconGateway"/>, which serialises them
/// onto the single RCON socket, so the extra read costs one command a minute.</para>
/// </summary>

...
/// <summary>
/// Runs a world sweep on every full hour (:00) over RCON: wipes all corpses and clears AI. Two minutes
/// before each wipe (:58) it broadcasts a warning so players can get clear.
///
/// The auto-sweep only runs when the server is below <see cref="PopulationGate"/> players, so a busy
/// server is never disrupted mid-session — admins can still force one with <c>!wipeworld</c>.
/// </summary>
    ...
    ILogger<WorldCleanupService> logger) : BackgroundService
{
    ...
    private async Task SafeAnnounceAsync()
    {
        try
        {
            await rcon.Announce("Server cleanup in 2 minutes: all corpses will be wiped and AI reset.");
        }
        catch (Exception ex)
        {

...
/// <summary>
/// The purge saga, and specifically the thing it must never do.
///
/// <para>The negative case is the feature here. An account deletion that hangs at
/// <c>PurgeInProgress</c> because Bots was never deployed is a GDPR erasure that silently did not
/// happen; the deadline exists so that becomes an alert naming Bots. The other half of the same
/// requirement is that the deadline must <b>not</b> resolve the saga - a purge that marked itself
/// complete would report an erasure that did not occur, which is strictly worse than hanging - so
/// there are assertions for both directions.</para>
/// </summary>
[TestFixture]

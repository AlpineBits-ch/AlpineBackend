...
public class PasswordResetEndpointTests
{
    ...
    /// <summary>
    /// Regression test for a bug found while writing these tests: EmailService (shared Messaging
    /// infra) never initializes its Graph client when AUTH_REQUIRE_USER_EMAIL_VERIFICATION=false
    /// (see Messaging/EmailService.cs's constructor), so SendEmailAsync threw a
    /// NullReferenceException for any existing account - unlike the welcome-email path
    /// (UserCreatedHandler), which already guards against this by skipping the call entirely under
    /// that flag.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// The request route always answered 202, but reset-password gave the game away on the way
    /// back: an unknown address got "Invalid code" where a live account with nothing cached got
    /// "Invalid or expired code" and one that had burned its attempts got "Too many incorrect
    /// attempts".
    /// </summary>
    [Test]

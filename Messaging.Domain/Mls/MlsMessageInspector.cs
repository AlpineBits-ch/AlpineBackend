...
/// <summary>RFC 9420 §6 <c>ContentType</c>. Carried in the clear in both PublicMessage and
/// PrivateMessage - a PrivateMessage encrypts the content, not the label on it.</summary>
...
/// <summary>
/// Reads the parts of an MLS message that are in the clear.
///
/// <para><b>Why the server parses anything at all.</b> It used to take the client's word for whether
/// a payload was a proposal, and advance the group's epoch on that word. A member could therefore
/// publish a Remove <i>proposal</i> - or 40 bytes of noise - flagged <c>isProposal: false</c> at
/// <c>epoch+1</c>, and the server would move <c>active.Epoch</c> to a number no honest client can
/// ever reach. Every subsequent honest commit then 409s, permanently, and the only way out is a full
/// re-key. On a channel that took nothing but <c>ViewChannel</c>.</para>
///
/// <para><b>What is legitimately readable.</b> RFC 9420 frames both message kinds with the group id,
/// the epoch and the content type outside the encrypted portion - a PrivateMessage hides the content,
/// not the fact that it <i>is</i> a commit. So the server can check that a payload is a well-formed
/// MLS message for the group it claims, and that its self-declared kind matches its actual one,
/// without holding a single group secret and without pretending to validate anything cryptographic.
/// That is the whole extent of it: nothing here verifies a signature, a MAC, or a tree.</para>
///
/// <para>Written against the wire format rather than against a library because the server has no MLS
/// implementation and should not acquire one for a header read. Structure per RFC 9420 §6.1
/// (<c>MLSMessage</c>), §6.2 (<c>FramedContent</c> inside <c>PublicMessage</c>) and §6.3.2
/// (<c>PrivateMessage</c>).</para>
/// </summary>
public static class MlsMessageInspector
{
    ...
    /// <summary>
    /// Parses the readable header, or returns false with a reason.
    ///
    /// <para>Failure means "this is not an MLS message", which is a thing the server may refuse
    /// outright: no honest client produces one, and accepting them is what lets a member wedge a
    /// group with noise.</para>
    /// </summary>

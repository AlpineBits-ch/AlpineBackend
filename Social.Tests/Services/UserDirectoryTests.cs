...
/// <summary>
/// The discoverability chokepoint (privacy spec T2-16).
///
/// <para>These tests exist because of what the T2-16 audit found: there is no lookup-by-email and no
/// lookup-by-phone anywhere in the product, so <c>DiscoverableByEmail</c> and
/// <c>DiscoverableByPhone</c> gated nothing at all. The response was not to invent an endpoint but to
/// make <see cref="UserDirectory"/> the one place a user is found by a typed identifier, with the
/// flag applied per key. What is pinned here is therefore mostly a <i>shape</i>: that each key reads
/// its own flag, that an unknown key fails closed, that email and phone resolve nothing today, and
/// that "not discoverable" is indistinguishable from "no such user".</para>
/// </summary>
[TestFixture]
public class UserDirectoryTests
{
    ...
    {
        // This is the executable form of the T2-16 finding: Social's Profile stores neither an email
        // nor a phone number, and no service in the solution offers a lookup by either.
        //
        // If you are here because you just added one: implement it in UserDirectory.ResolveAsync and
        // nowhere else. FindAsync already applies DiscoverableByEmail/ByPhone to whatever that
        // returns, so the gate comes with the lookup - which is the entire reason this class exists.
        await AddProfile("user-1", "target");

...
///
/// Root cause: ConversationCreatedHandler has an empty foreach body, so the permission cache
/// is never rebuilt after a conversation is created. FriendRequestAcceptedHandler warms the
/// cache (without the new conversation) just before the user creates one, so HasPermission
/// returns false indefinitely until the 10-minute TTL expires.
/// </summary>
[TestFixture]
public class ConversationCreatedHandlerTests
{
    ...
    private void WarmCacheWithoutNewConversation(string userId)
    {
        ...
        // At the time of friend acceptance no conversation exists yet, so the
        // rebuilt cache will NOT contain ConversationId.
    ...
    /// A conversation is then created. The handler must rebuild the cache so that both members
    /// can immediately access the conversation.
    ///
    /// This test currently FAILS because the foreach body in ConversationCreatedHandler is empty.
    /// </summary>
    [Test]

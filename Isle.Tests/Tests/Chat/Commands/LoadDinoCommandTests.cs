...
public class LoadDinoCommandTests
{
    ...
    {
        // Storage.StoreDino itself enforces the growth threshold at store time, so a below-threshold
        // slot can only exist if the threshold was raised after the slot was stored. Simulate that by
        // mutating the already-persisted, tracked entity rather than trying to store one directly below
        // threshold (which Storage.StoreDino would reject).

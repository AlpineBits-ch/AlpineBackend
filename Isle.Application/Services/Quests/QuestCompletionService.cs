...
/// <summary>
/// How an <c>Exploration</c> or <c>Hunt</c> quest is satisfied, paid and closed. The non-bounty
/// counterpart to <see cref="BountyService"/>, and the reason those quests stopped reporting "went
/// unclaimed" on every single run: before this existed nothing could ever mark one completed, so the
/// director spawned them, the clock ran out, and the broadcast was technically accurate.
///
/// <para>The two types are satisfied by different things, taken from the seeded templates rather than
/// invented here:</para>
/// <list type="bullet">
/// <item><b>Exploration</b> — "travel to the marked region and stay a while", every reward row at
/// <see cref="RankRequirement.AllParticipants"/>. Satisfying it is not an event the game server ever
/// reports; it is the absence of anyone leaving, which is why it is sampled into
/// <see cref="QuestProgressLedger"/> and settled at expiry rather than closed the moment somebody
/// arrives. Running the full window is also what lets latecomers count.</item>
/// <item><b>Hunt</b> — "prey has been sighted, go and eat", every row at
/// <see cref="RankRequirement.Winner"/>. One kill in the right region ends it, immediately, for the
/// player who made it.</item>
/// </list>
///
/// <para><b>Matching is by region id, not by geofence.</b> Deliberate, and worth knowing before
/// changing it: every seeded fence is a circle of radius 0 so
/// <see cref="GeoFenceData.Contains"/> is false for every position on the map, and the fence does its
/// maths on XZ while the roster and <see cref="RegionMap"/> use XY — the mismatch
/// <c>QuestSpawner.ResolveCoordinates</c> already flags. Wiring the fence in would look right and
/// silently qualify nobody. <see cref="RosterEntry.RegionId"/> is already resolved per tick, so the
/// region path costs no geometry and is honest about its precision.</para>
/// </summary>
    ...
    ILogger<QuestCompletionService> logger)
{
    ...
    /// <summary>
    /// What a payout actually did, as opposed to what it was owed. Same shape and same reasoning as
    /// <c>BountyService.Payout</c>: qualifying only means the ledger saw you, and whether anything
    /// reached you depends on the granter finding a live pawn. Announcing the qualified count instead of
    /// this one is how a server tells the lobby that four players were paid when none of them were.
    /// </summary>
    /// <param name="WinnerLines">The winner's payout lines, for the hunt claim whisper. Empty otherwise.</param>
    /// <param name="PaidPlayerIds">Exactly the players something reached, in the order they were paid.</param>
    ...
    ///
    /// <para>At <see cref="QuestProgressService"/>'s cadence this is roughly a minute. Enough to mean
    /// "went there" rather than "ran through on the way somewhere", short enough that the trip does not
    /// become a commitment — the template asks players to stake a claim, not to stand guard. This is the
    /// number to move if the qualified-vs-total debug line shows real visitors falling short.</para>
    /// </summary>
    ...
    /// <summary>
    /// Fallback payout for an exploration template authored without rewards. Mirrors the seeded shape:
    /// never XP alone, and everything at the participation tier, because an exploration has no contest
    /// in it — everyone who made the trip did the same thing.
    /// </summary>
    ...
    /// <summary>
    /// Fallback payout for a hunt template authored without rewards. Winner-only, and weighted toward
    /// the state the killer is left in rather than the number they bank — they just took a fight.
    /// </summary>
    ...
    /// claim instead. Here for a template that authors a lesser tier — the payout would otherwise reach
    /// those players silently.
    /// </summary>
    ...
    /// <summary>
    /// Credits one roster sample to every player standing in an open quest's region. Driven by
    /// <see cref="QuestProgressService"/>, and reads <see cref="WorldRosterCache"/> only — no RCON, no
    /// geometry, one Redis batch per quest with anyone in it.
    ///
    /// <para>Hunts are sampled too even though nothing resolves off it: it costs one hash write and it
    /// is what puts a headcount on <c>!quests</c>, which is how a player can tell the quest is
    /// registering them at all. Do not extend <see cref="ResolveDueQuestsAsync"/> to pay a hunt off
    /// presence on the strength of the ledger being populated — a hunt is won by the kill.</para>
    /// </summary>
    ...
    /// <summary>
    /// A player made a kill. Completes an open hunt if they made it in the right region, and pays them
    /// the winner's share. No-op on every other kill, which is the overwhelming majority of them.
    ///
    /// <para>The kill position comes from the killer's roster entry, so it can be up to one roster tick
    /// stale. Accepted rather than worked around: the killfeed carries no position at all, and regions
    /// are large enough that a player who was in one 30 seconds ago was almost certainly hunting in it.
    /// The failure mode is a hunt paying out to someone who crossed the border mid-fight, which is a
    /// generous reading of a kill they did make, not a payout to someone who did nothing.</para>
    /// </summary>
    ...
    /// <summary>
    /// Settles every non-bounty instance whose window has run out. Replaces the old
    /// <c>QuestSpawner.ExpireDueQuestsAsync</c>, which only ever knew how to expire them.
    ///
    /// <para>An exploration closes <see cref="QuestInstanceState.Completed"/> if anybody cleared the
    /// dwell bar and <see cref="QuestInstanceState.Expired"/> if nobody did — the "went unclaimed" line
    /// survives, but now it only goes out when it is true. A hunt that reaches its expiry was not
    /// claimed by definition, so it always expires.</para>
    ///
    /// <para>Bounty expiry stays <see cref="BountyService.ExpireDueBountiesAsync"/>'s job: it has to
    /// unmark the target and restore their skin, which is more than a state flip.</para>
    /// </summary>
    ...
    /// <summary>
    /// Shared teardown for a completed instance: persist, drop the ledger, publish. Every completion
    /// path goes through it so no path can be the one that forgets to clear Redis — a stale presence
    /// hash outlives the quest by twelve hours and would hand the next instance a crowd that never came.
    /// </summary>
    ...
    /// <summary>
    /// Resolves the ledger's Steam ids to players and gives each the tier their showing earns: earliest
    /// arrival first, then <see cref="TopTierSize"/> deep at <see cref="RankRequirement.Top3"/>, everyone
    /// else at <see cref="RankRequirement.AllParticipants"/>.
    ///
    /// <para>Ranking an exploration at all is invisible against the seeded templates, whose rows are all
    /// at the participation tier — the tiers nest downwards, so first-there and last-there are paid
    /// identically today. It exists so an admin <i>can</i> author a first-there bonus without the
    /// resolution needing to change.</para>
    /// </summary>
    ...
    /// <summary>
    /// Pays everyone the resolved quest owes and whispers each of them what they got — nothing else ever
    /// would, since the broadcast is written for the lobby.
    ///
    /// <para>Returns what the payout actually did rather than what it was owed; see <see cref="Payout"/>
    /// for why the caller must announce that number.</para>
    /// </summary>

...
{
    // Test projects contain fakes and assertions that would pollute the contract. The generator
    // itself would otherwise scan its own source.
...
//
// The artifacts are committed rather than generated during the Docker build: they are
// deterministic from source, so building them in the image would mean restoring and
// design-time-building the whole solution on every image build. Committing them also puts a
// change to the public contract in the pull-request diff, where it can be reviewed.

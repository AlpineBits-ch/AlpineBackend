...
/// <summary>
/// Builds Isle's privacy stack over a substituted <see cref="IMessageBus"/>: a
/// <see cref="PrivacySettingsCache"/> on a <see cref="FakeDistributedCache"/> plus the
/// <see cref="PositionalVoiceConsent"/> gate on top of it.
///
/// <para>Defaults are the <i>product</i> defaults, not the fail-closed ones - a test that wants the
/// restrictive path has to say so, otherwise "refuses" would be indistinguishable from "the test
/// forgot to configure Identity".</para>
/// </summary>

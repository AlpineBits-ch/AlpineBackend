...
/// branch, and mails the account holder - and that the refusals which survive are decided before the
/// address is ever looked up.
///
/// <para>The HTTP tests in <c>UserRegistrationTests</c> prove the two answers are byte-identical.
/// They cannot prove <i>why</i>, and "identical because both branches refuse" would pass them. These
/// pin the mechanism.</para>
/// </summary>
[TestFixture]
public class RegistrationEnumerationTests
{
    ...
    /// <summary>Records what would have been mailed instead of mailing it. <see cref="AccountEmailDispatcher"/>
    /// declares its queue methods virtual for exactly this.</summary>
    ...
    {
        // The common honest case: signed up, never found the code, signed up again. A notice about
        // themselves leaves them exactly where they were; the code is the only version of this that
        // terminates. Invisible to the caller either way - the response is already written.
    ...
    public async Task LosingTheRaceToAConcurrentSignup_LandsOnTheSameAcceptedAnswer()
    {
        ...
        // account between this one's lookup and its insert. Rare - and it still has to be the
        // uniform answer, or the oracle survives at a low duty cycle where nothing would notice it.

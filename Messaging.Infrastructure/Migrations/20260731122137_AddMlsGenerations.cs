...
namespace Messaging.Infrastructure.Migrations
{
    ...
    public partial class AddMlsGenerations : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // the context is plaintext, and starts refusing the ciphertext its clients are correctly
            // still producing.
            //
            // Ids are minted here rather than by the application's Ksuid generator: these rows are
            // opaque and only ever looked up by (context_id, generation), so a uuid body behind the
            // usual prefix is indistinguishable in use.
            //
            // activated_at is the conversation's own creation time rather than now(), so the toggle
            // cooldown does not spuriously block a disable for the first 30 seconds after deploy.

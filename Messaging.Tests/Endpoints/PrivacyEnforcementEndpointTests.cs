...
public class PrivacyEnforcementEndpointTests
{
    ...
    {
        // Existing conversations are not retroactively closed. Somebody in the group flipping their
        // global DM policy must not stop the group working for everyone else - what a block buys
        // them inside a group is silence, not a broken room (see MessageCreatedHandler).
    ...
    {
        // The default deployment. The control exists but nothing classifies, so nothing is blocked -
        // and the recipient's filter is never even resolved.
        await SeedConversation("conv-dm", "user-1", "user-2");
        await SeedAttachment("atac_1", "user-1");

...
public class EchoRealtimeHub(ILogger<EchoRealtimeHub> logger, IMessageBus bus) : Hub
{
    ...
    /// <summary>
    /// SignalR group name for one specific device of one specific user. Downstream services
    /// (via <see cref="IHubContext{EchoRealtimeHub}"/>) send to this group to reach exactly one
    /// device instead of every open session for the user - e.g. to kick the device that just
    /// got superseded by a call/voice-channel takeover on another device.
    /// </summary>

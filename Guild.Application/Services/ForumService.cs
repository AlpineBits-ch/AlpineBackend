...
/// <summary>Outcome of a tag-set write. Split three ways rather than throwing because the caller
/// has to map each to a different status code, and a rejected tag write is an ordinary outcome
/// (a full picker, a moderated chip) rather than an exceptional one.</summary>
...
public class ForumService(MicroserviceContext ctx)
{
    ...
    /// <summary>Applied tag ids per post, ordered by tag Position, for a page of posts. One query
    /// for the whole page rather than one per post.</summary>
    ...
    /// <summary>Replaces a post's tag set. Validates the cap, forum membership, and the require-tag
    /// rule, then gates moderated tags on the caller being a moderator - checked against the
    /// *difference*, not the whole set, so a post author can re-save their own tags without
    /// tripping over a moderated tag someone else applied.</summary>
    ...
    // ── Keyset cursors ───────────────────────────────────────────────────────
    // "{pinned}|{ticks}|{id}", base64url. Opaque to the client by contract; the id tiebreak is
    // what makes the ordering total, so two posts sharing a timestamp can't shadow each other.

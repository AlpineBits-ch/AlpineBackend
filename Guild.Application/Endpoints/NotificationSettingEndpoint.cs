...
/// <summary>
/// A member's own notification preferences. Every route here acts on the caller and only the
/// caller - there is no permission check anywhere in this file beyond "are you a member of this
/// guild", because these are personal settings and no permission could ever let one member change
/// another's.
/// </summary>
[Authorize]
public class NotificationSettingEndpoint
{
    /// <summary>Stand-in for "muted until I say otherwise". Far enough out to never arrive,
    /// close enough to remain a valid timestamp in every store this passes through.</summary>

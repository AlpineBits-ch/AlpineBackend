...
/// <summary>The chore rota on a <see cref="ChannelType.Chores"/> channel.
///
/// Rotation and the fairness balance live in <see cref="ChoreRotationService"/>; this is the HTTP
/// surface over it.</summary>
[Authorize]
public class ChoreEndpoint
{
        ...
        ClaimsPrincipal user)
    {
        ...
        if (completed)
        {
            ...
            // Whoever actually did it, which is frequently not who it was assigned to. The balance
            // still credits AssignedUserId; crediting the doer would let one flatmate farm the
            // fairness ledger by doing everyone's easy chores.
    ...
    /// <summary>Hands an occurrence to whoever currently has the lightest load. The point is that
    /// "I can't do the bins tonight" resolves itself without anybody having to negotiate.</summary>
    ...
    /// <summary>Who is actually carrying the house. This is the number that turns "you never
    /// clean" from an argument into a fact.</summary>

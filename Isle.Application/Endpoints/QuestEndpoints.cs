...
/// <summary>
/// Read-only quest surface for the companion website. Anonymous on purpose: everything here is
/// already broadcast to every player in chat, so there is nothing to protect.
///
/// <para>Bounties intentionally expose the position captured when the target was marked and never a
/// live one — same rule as <c>!bounty</c>. A web page that live-tracked a marked player would turn the
/// hunt into a map overlay.</para>
/// </summary>
public static class QuestEndpoints
{
    ...
    /// <summary>
    /// The region table with live headcounts — the data a map overlay needs. Coordinates are the
    /// current placeholder values and will be wrong until the region table is rebuilt.
    /// </summary>

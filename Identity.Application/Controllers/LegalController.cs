...
///
/// <para>Before this existed there was no record anywhere that a user had ever accepted anything -
/// not the terms, not the privacy policy, not a version of either. "The user agreed" was an
/// assumption about a screen the client may or may not have shown.</para>
/// </summary>
[ApiController]
[Route("api/v1/legal")]
...
{
    /// <summary>
    /// The current version of each published document.
    ///
    /// <para><b>Anonymous.</b> The terms have to be readable before there is an account to read them
    /// with - a registration screen that cannot show what is being agreed to is not consent - and
    /// nothing here is specific to any caller.</para>
    /// </summary>
    [AllowAnonymous]
    [HttpGet("documents")]
    ...
    /// <summary>
    /// The bytes of one published version (T1-12).
    ///
    /// <para><b>This is the URL <c>LegalDocument.Url</c> points at</b>, and it serves exactly the
    /// bytes that were hashed into <c>ContentHash</c> - which is what makes the hash worth storing.
    /// Served as text/markdown rather than rendered HTML: the hash is over the source, and rendering
    /// server-side would put a template between the hash and what the user reads.</para>
    ///
    /// <para>Anonymous, and cacheable. A legal document at a given version is immutable by policy;
    /// if it changes, that is a new version at a new URL.</para>
    /// </summary>
    [AllowAnonymous]
    ...
    ///
    /// <para><b>The version must be one that has actually been published</b>, and must be current or
    /// past-effective. A client that could name any string would be able to satisfy an outstanding
    /// consent by inventing a version number, and a client that could accept a future-dated version
    /// would be agreeing to something that is not yet in force.</para>
    ///
    /// <para>Idempotent - re-posting an acceptance returns the original record with its original
    /// timestamp, because the evidence is of when the user actually agreed, not of the last retry.</para>
    /// </summary>
    [Authorize]
    [HttpPost("consents")]
    ...
    public async Task<IActionResult> RecordConsentAsync([FromBody] RecordConsentRequest request, CancellationToken ct)
    {
        ...
        // Only audit a consent that is actually new. A retry of a request whose response was lost is
        // not a second decision, and an append-only log that records it as one makes the account look
        // like it accepted the same terms twice.
    ...
    ///
    /// <para>A 405 with no body would leave a client author guessing whether the route was missing or
    /// the operation was forbidden. T1-10 is specific: Terms and Privacy are not withdrawable while
    /// the account is active - the withdrawal path there is account deletion, <b>and the client
    /// should say so</b>. The optional consents (data collection, personalization, voice recording)
    /// are a different thing entirely and are withdrawn instantly through
    /// <c>PATCH /api/v1/privacy-settings</c>, which never degrades core service.</para>
    /// </summary>
    [Authorize]

...
/// <summary>
/// Serves the re-hosted preview images that <c>proxy_url</c> points at.
///
/// <para><b>Deliberately unauthenticated</b>, unlike every other media route in the product. These
/// URLs are embedded in messages and rendered by <c>&lt;img&gt;</c> tags, link previews in
/// notifications, and OS-level share sheets - none of which carry a bearer token. Requiring auth
/// would mean no preview image ever loads.</para>
///
/// <para>That is safe only because of what can be here: the object key is the SHA-256 of a public
/// http(s) URL, and the only writer is the unfurler, which cannot fetch a private address at all
/// (see <c>OutboundAddressGuard</c>). Everything served here was already public on the open web to
/// anyone who had the source URL. Nothing user-uploaded and nothing permission-scoped shares this
/// prefix - those live behind <c>AttachmentController</c>, which does check.</para>
/// </summary>
[ApiController]
[AllowAnonymous]

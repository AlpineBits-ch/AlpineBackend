...
/// <summary>
/// Pins the declared-path to public-path mapping the docs aggregator applies.
///
/// <para>This is the part of the docs pipeline most likely to be silently wrong. A service declares
/// <c>/api/v1/inbox/unread</c>; the gateway rewrites <c>/api/v1/guild/{**}</c> to
/// <c>/api/v1/{**}</c>, so the public URL is <c>/api/v1/guild/inbox/unread</c>. Getting that
/// backwards publishes an address that 404s, and nothing else in the system would notice - the docs
/// would render perfectly and every URL in them would be wrong.</para>
///
/// <para>Pure string logic, so these run in milliseconds with no containers, unlike the rest of this
/// project.</para>
/// </summary>
[TestFixture]
[Category("Unit")]
public class DocsPathMappingTests
{
    ...
    /// <summary>Identity and Federation declare several routes without a leading slash. ASP.NET
    /// normalises them; so must the mapping, or they would fall through as unreachable.</summary>
    ...
    /// <summary>The document endpoint the aggregator itself reads. No proxy route reaches it, so it
    /// must never appear in the public docs.</summary>
    [Test]

...
/// <summary>Reading and moderating forum posts. Creating one still goes through
/// ThreadEndpoint.CreateThreadAsync - a post *is* a thread, and splitting creation across two
/// endpoints by parent type would duplicate the permission and initial-message logic.</summary>
[Authorize]
public class ForumPostEndpoint
{
        ...
        string? archived = null, int? limit = null, string? cursor = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(cursor))
        {
            ...
            // The id leg is what makes the ordering total - without it, two posts sharing a
            // timestamp would let one shadow the other across a page boundary.
        ...
        [NotBody] IMessageBus bus, [NotBody] ClaimsPrincipal user)
    {
        ...
        // The author can retag their own post; anyone else needs ManageAnyThread. Moderated tags
        // are gated separately inside SetPostTagsAsync, against the added/removed difference.

...
public class DiscordImportEndpointTests
{
    ...
    // ── Authorization on the link routes ─────────────────────────────────────
    // These three used to take no caller identity at all, relying on a gateway check that does not
    // exist (no YARP route in this repo sets an AuthorizationPolicy). Any authenticated user could
    // therefore enumerate any guild's Discord link, pause its sync, or revoke it - which also makes
    // the import bot leave that guild's Discord server.

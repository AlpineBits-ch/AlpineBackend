...
/// <summary>
/// Manual control over the quest giver.
///
/// <para>The <c>bounty</c> verb is the reason this exists: when an admin sees somebody misbehaving,
/// they can mark them on the spot with a custom duration and payout. It skips every spree threshold
/// deliberately — the automatic gates exist to judge whether a streak is impressive, which is not the
/// question being asked when an admin does it by hand.</para>
/// </summary>
    ...
    PopulationHeatmap heatmap) : ChatCommand
{
    ...
    /// !questadmin end &lt;Q-id&gt;
    ///
    /// <para>Takes the friendly id an admin can read off <c>list</c>, and still accepts a raw ksuid so
    /// an id copied out of a log line keeps working.</para>
    /// </summary>

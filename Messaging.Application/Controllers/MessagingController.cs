...
public class MessagingController(IMessageRepository repo, ILogger<MessagingController> logger, IMessageBus bus, ConversationPermissionService conversationPermissionService) : ControllerBase
{
    ...
    ///
    /// Only one of before/after/around is honoured; if a client sends more than one they are
    /// preferred in that order rather than rejected, since the combination has no coherent meaning
    /// and failing the request would be a worse outcome than picking the most common intent.
    /// </summary>
        ...
        [FromQuery] string? before = null, [FromQuery] string? after = null, [FromQuery] string? around = null)
    {
        ...
        try
        {
            ...
            // Response shape is identical on both paths, so a client can adopt cursors without
            // any parsing change - it just stops sending offset. `hasMore` is inferable the same
            // way it always was: a full page back means there is probably another.

...
///
/// <para>This is the site that mattered most, because it is a push: the member-list projection at
/// least required somebody to ask, while this delivered the leak to every online member of every
/// guild the user was in, unprompted, the moment they went invisible.</para>
/// </summary>
[TestFixture]
public class GuildPresenceBroadcastTests
{
    ...
    {
        // Fail closed. The subject still hears about their own change, because that pair can never
        // be blocked.
        _bus.ClearResponses();

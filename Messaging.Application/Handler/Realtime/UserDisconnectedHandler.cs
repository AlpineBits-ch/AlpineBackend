...
public class UserDisconnectedHandler
{
        ...
        LockedJsonCacheStore callStore, IHubContext<EchoRealtimeHub> hub)
    {
        ...
        // VoiceHub.OnDisconnectedAsync: leave any active call. Routed through Call.Leave (same
        // path as the explicit /leave endpoint) rather than just notifying, so an app kill/
        // network drop behaves like a real hangup: the call keeps running for other connected
        // participants, and dropping to one survivor starts the alone-timeout grace period
        // instead of the old behavior of never updating participant state at all.

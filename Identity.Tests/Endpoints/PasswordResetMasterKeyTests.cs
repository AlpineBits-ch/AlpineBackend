...
/// <summary>What a password reset does to the account master key.</summary>
[TestFixture]
public class PasswordResetMasterKeyTests
{
    ...
    public async Task ResetPassword_WithoutARecoveryCodeWrapping_ReportsTheHistoryAsLost()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            // This is a completed loss, not a warning about the future.
    ...
    public async Task Rewrapping_AfterAReset_RestoresThePasswordWrappingWithoutRotating()
    {
        ...
        // The reset mints the single-use permit for the re-wrap it just made necessary.
    ...
    public async Task Rewrapping_AtTheWrongVersion_IsRefused()
    {
        ...
        // The password and the matching verifier are both supplied deliberately: the version check
        // is the last of the three gates, so a request that fails an earlier one would return 403
        // and never reach it.
        await Host.Scenario(x =>
        {
            x.WithBearerToken(token);

...
///
/// As with slash commands, there is no real Discord client in this system: venta's own client is
/// what renders a bot's buttons and what tells the backend one was pressed. This endpoint turns
/// that into the MESSAGE_COMPONENT interaction the bot's library expects, over the same gateway
/// dispatch and the same PendingInteractionStore the command path already uses.
/// </summary>
[Authorize]
public class ComponentInteractionEndpoint
{
    ...
    ///
    /// The bot's reply arrives on its own gateway connection as callback type 8, which may land on
    /// a different pod entirely - so the reply is parked in Redis and this request polls for it
    /// rather than awaiting an in-process handle. Polling with a short deadline (rather than
    /// blocking on a pub/sub subscription) keeps the failure mode boring: a bot that never answers
    /// costs one request its timeout and returns an empty list, which renders as "no suggestions"
        ...
        [NotBody] GatewayConnectionRegistry registry, [NotBody] PendingInteractionStore pendingStore)
    {
        ...
        // Without it a caller could name any enabled bot with a global command and have this guild's
        // id, channel id and the invoking member's profile dispatched to a bot that was never
        // installed here.
        var installed = await ctx.BotInstallations.AnyAsync(i => i.BotApplicationId == app.Id && i.GuildId == guildId);
        if (!installed) return Results.NotFound("Bot is not installed in this guild.");
        ...
        interaction.Data = new InteractionDataPayload
        {
            ...
            Options = dto.Options.Select(o => new InteractionOptionPayload
            {
                ...
                // Undefined - which System.Text.Json refuses to write. Substituting an empty
                // string keeps a half-typed option (exactly what autocomplete is *for*) from
    ...
    /// <summary>Deliberately short. This budget is spent with a user waiting on a keystroke, and
    /// Discord's own autocomplete deadline is 3 seconds.</summary>
    private const int AutocompleteTimeoutSeconds = 3;

    private const int AutocompletePollMilliseconds = 50;

    /// <summary>A writable stand-in for an absent option value. Parsed once - JsonDocument.Parse
    /// per request for a constant would be pure waste.</summary>
    ...
    /// on InteractionPayload for why each has to be present.</summary>
...
public class InvokeComponentDto
{
    ...
    /// <summary>The pressed component's type - 2 for a button, 3+ for the selects. Qualified
    /// because the property name shadows the constants class it defaults from.</summary>

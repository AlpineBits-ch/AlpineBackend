...
/// <summary>
/// Emits an AsyncAPI 3 document for the single realtime hub.
///
/// <para>Direction is from the <b>client's</b> point of view, which is the audience: what the server
/// pushes is a <c>receive</c> operation, what the client invokes is <c>send</c>. Both travel over
/// one SignalR connection at <c>/api/v1/ws/hub</c>.</para>
/// </summary>
internal static class AsyncApiWriter
{
    public static string Write(IReadOnlyList<OutboundSite> outbound, IReadOnlyList<InboundMethod> inbound)
    {
        ...
        foreach (var method in inbound.OrderBy(i => i.EventName, StringComparer.Ordinal))
        {
            ...
            // SignalR hub methods take a positional argument list, not a single object. Modelled as
            // an array so the docs match the invocation, rather than implying a JSON body.

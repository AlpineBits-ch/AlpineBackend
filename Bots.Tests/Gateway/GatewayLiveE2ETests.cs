...
public class GatewayLiveE2ETests
{
    ...
    /// <summary>
    /// Full round trip: the bot sends itself a message over the existing REST compat surface, then
    /// the SAME Gateway connection must receive a MESSAGE_CREATE dispatch for it - proves the Guild
    /// -> Bots.Application dispatch-fan-out path actually delivers, not just that the handshake
    /// completes.
    /// </summary>
    [Test]

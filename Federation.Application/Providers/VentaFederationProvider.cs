...
public class VentaFederationProvider : IFederationProvider
{
    ...
    // side ends up with a permanent gap with no indication anything went wrong. Now records
    // delivery outcome so FederationOutboundRetryService can retry it, and a permanently-failing
    // event is at least visible (Attempts keeps climbing) instead of silently vanishing.
    ...
    private async Task FireEventAsync(FederationEvent @event)
    {
        ...
        // Previously published FederationInboundEventReady here, which had no subscriber
        // anywhere - a successfully DAG-resolved inbound event was recorded and then silently
        // dropped. Now dispatches the typed Federation.Contracts materialization command the
        // owning service (Guild/Messaging/Social) actually subscribes to.

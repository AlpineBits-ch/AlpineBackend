...
/// <summary>
/// Social's read side for the privacy record Identity owns (privacy spec §1.4).
///
/// <para>Redis-backed rather than in-memory, and keyed <c>privacy_settings:user_id:{id}</c> exactly
/// as the spec names it. Social reads these on every friend request and every profile projection,
/// so a per-pod in-memory copy would mean a user's "nobody may add me" toggle silently keeps not
/// applying on whichever pod happens to serve the next request. Follows the same
/// IDistributedCache-plus-bus shape as the existing <c>integration_profile:user_id:{...}</c>
/// cache in <c>GetProfileByUserIdHandler</c>.</para>
///
/// <para><b>Fails closed.</b> If the cache misses and the bus call fails, callers get
/// <see cref="RestrictiveDefaults"/> - <c>Friends</c> for DMs and <c>Nobody</c> for everything else -
/// never <c>Everyone</c>. An outage must not turn into an open door, which is the whole reason this
/// class exists instead of an inline <c>InvokeAsync</c> at each call site.</para>
/// </summary>
public class PrivacySettingsCache(IDistributedCache cache, IMessageBus bus, ILogger<PrivacySettingsCache> logger)
{
    ...
    /// <summary>
    /// What a caller gets when the record cannot be resolved. Every contactability and visibility
    /// field is set to its most restrictive value; the consent booleans are false because consent
    /// that cannot be confirmed has not been given.
    /// </summary>
    ...
    /// <summary>
    /// Resolves a whole set at once. Cached ids are served from Redis, the remainder go out as a
    /// single batched <see cref="GetUserPrivacySettingsRequest"/> - the batching is the reason the
    /// contract takes <c>UserIds</c> rather than <c>UserId</c>.
    /// </summary>
    ...
    /// <summary>Drops one user's key. Called by the <c>UserPrivacySettingsChangedEvent</c> handler;
    /// eviction rather than overwrite, because the event deliberately carries no values.</summary>

...
| **E11** | `POST /api/v1/conversations` is idempotent on the member set. A request whose members, name and encryption state match a local conversation the caller is already in returns **302** with that conversation instead of 200 with a new one. Nothing of the discarded attempt is stored - no second generation, and the `deviceWelcomes` are dropped, because they are sealed to a group the returned conversation does not use. A client that gets a 302 on an encrypted create must therefore join the group named by the returned `mlsGroupId` rather than keep the one it just built. `Location` is the **relative** reference `conversations/{id}` - the gateway strips its own `/api/v1/messaging` prefix and forwards no replacement, so the service cannot emit an absolute public URL. Nothing re-POSTs to it: only 307 and 308 preserve the method. Browser clients will not observe the 302 at all - `fetch`/XHR follow it, rewrite the POST to a GET and surface 200 from `GET .../conversations/{id}`, which carries the same conversation. Dart's `HttpClient` auto-follows only GET and HEAD, so venta-mobile sees the 302 itself. | Echo, both clients |

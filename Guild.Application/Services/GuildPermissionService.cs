    ...
    ILogger<GuildPermissionService> logger)
{
        ...
        string userId, string guildId)
    {
        ...
        // A muted (timed-out) member keeps their other permissions but loses the ability
    ...
    /// <summary>
    /// Guards against privilege escalation: an actor may only grant permission bits
    /// they themselves currently hold (guild owner is exempt, matching the Superadmin
    /// short-circuit already used throughout this service). Apply this whenever a
    /// non-owner can set arbitrary Allow/Permissions bits on a role or overwrite.
    /// </summary>
    ...
    /// <summary>
    /// Highest role Position the user holds in this guild. Guild owner is treated as
    /// above every role (int.MaxValue). A member with no roles returns int.MinValue.
    /// </summary>
    ...
    /// <summary>
    /// True if the actor's highest role outranks the target role (or the actor is
    /// guild owner). Used to stop a ManagePermissions holder from editing/deleting/
    /// assigning a role positioned at or above their own highest role.
    /// </summary>
    ...
    /// <summary>
    /// True if the actor outranks the target member (or is guild owner). The guild
    /// owner can never be moderated. Used to gate ban/kick/mute against targeting a
    /// member with an equal-or-higher role.
    /// </summary>

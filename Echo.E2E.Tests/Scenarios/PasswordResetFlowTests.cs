...
/// <summary>
/// Proves the "forgot password" flow end to end: requesting a reset generates a real short code
/// in the same shared Redis instance Identity itself reads back from (read directly here via
/// StackExchange.Redis, the same "assert against real infra directly" pattern AccountDeletionFlowTests
/// uses for Postgres - there's no dev/fake email sink wired up in this harness, so the emailed
/// code has to be observed at its storage layer instead), resetting the password with that code,
/// and then confirming the OLD password no longer authenticates while the NEW one does via the
/// real <c>/connect/token</c> password grant.
///
/// This also exercises a real cross-service-adjacent bug this test discovered: PasswordResetEndpoint
/// unconditionally called EmailService.SendEmailAsync, which only constructs its Microsoft Graph
/// client when AUTH_REQUIRE_USER_EMAIL_VERIFICATION=true (see EmailService's constructor) - with
/// that flag false (as this harness and any similarly-configured deployment sets it, see
/// EchoTestStack.Common), sending would NullReferenceException. Fixed by guarding
/// EmailService.SendEmailAsync itself so it no-ops when email isn't configured, matching the
/// convention UserCreatedHandler already followed at its own call site.
/// </summary>
[TestFixture]
[Category("E2E")]

...
///
/// <para>Kept apart from <c>MessageCreatedHandlerTests</c>, which is about mention resolution and
/// the shape of the work. What is asserted here is the opposite - that certain people are
/// <i>absent</i> from the outputs, and that a payload is missing fields it would otherwise carry.</para>
/// </summary>
[TestFixture]
public class MessageCreatedPrivacyTests
{
    ...
    {
        // Restrictive default is HidePushContent = true. The content is withheld rather than sent
        // to someone who may have asked for it not to be.
        var hydrate = new GuildHydrateService(
            RedisTestFactory.CreateWithPresence(), NullLogger<GuildHydrateService>.Instance);

...
/// that is where a bug would actually live. The database-shaped ResolveForChannelAsync around it
/// only assembles the three inputs this takes.
/// </summary>
[TestFixture]
public class NotificationResolutionServiceTests
{
    ...
    public void Resolve_ChannelOverrideWithNullLevel_InheritsRatherThanResetting()
    {
        ...
        // change what level it is on. Treating null as "AllMessages" here would silently undo an
        // OnlyMentions preference the moment they muted anything.
        var result = NotificationResolutionService.Resolve(

...
/// <summary>
/// Covers the wire translation for <see cref="UserInterests"/>.
///
/// <para>The wire form is a lowercase name array rather than the serialized flags enum, and this is
/// the only place the two representations meet. Everything downstream - whether the client shows
/// its onboarding picker, and whether it asks for a master key at launch - is decided from what
/// comes out of here.</para>
/// </summary>
public class UserInterestsTests
{
    ...
    /// <summary>
    /// Refused whole, not partially accepted. Keeping the half we recognised would leave the account
    /// stored as having chosen something its client believes it did not, and that disagreement only
    /// surfaces much later as a launch sequence doing the wrong thing.
    /// </summary>
    [Test]

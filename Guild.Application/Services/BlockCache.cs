...
/// <summary>
/// Every block one user is party to, in either direction.
///
/// <para>Both directions are stored together on purpose: it is what makes a <b>single</b> cache key
/// answer the whole question for a fan-out. A presence broadcast, a mention fan-out and a typing
/// indicator each have one subject and many counterparties, so resolving the subject's state
/// resolves every pair in the batch - the counterparties are never looked up at all.</para>
/// </summary>
public sealed class UserBlockState
{
    ...
    /// <summary>Users who pressed block on this user. Never surfaced to them as a distinct
    /// state - it only ever removes things from a payload.</summary>
...
public sealed class BlockView
{
    ...
    /// <summary>A view over nobody. Everything is unresolved, so everything is treated as
    /// blocked - only useful as an explicit "we know nothing" in a test.</summary>
    ...
    /// <summary>A view that knows there are no blocks at all. Used by callers that have already
    /// established the pair is irrelevant.</summary>
    public static BlockView Empty { get; } =
        new(new Dictionary<string, UserBlockState?>(StringComparer.Ordinal));

    /// <summary>
    /// Whether a block exists between the pair, in <b>either</b> direction.
    ///
    /// <para>Symmetric on purpose. The spec requires two things of a block - the blocker receives
    /// nothing originating from the blocked user, and no realtime or presence event flows
    /// <i>between the pair</i> - and the second is bidirectional. Collapsing both into one
    /// predicate is also what keeps the blocked party from being able to detect the block by
    /// noticing that events still flow one way.</para>
    ///
    /// <para><b>Fails closed.</b> If neither side's state could be resolved the pair is reported
    /// blocked, because the alternative is delivering something to someone who asked never to
    /// receive it. One resolved side is enough: a resolved <see cref="UserBlockState"/> lists both
    /// directions, so it is complete for every pair that user is in.</para>
    /// </summary>
...
/// <summary>
/// Guild's read-through view of Social's block list (privacy spec T0-3).
///
/// <para>Same shape and same rules as <see cref="PrivacySettingsCache"/>: Redis-backed under
/// <c>blocks:user_id:{id}</c>, evicted by <c>UserBlockedEvent</c>/<c>UserUnblockedEvent</c>, TTL as
/// a backstop, fail closed. Blocking is the one control where a delay is directly harmful - someone
/// pressed block to stop something happening right now - which is why eviction is event-driven and
/// the TTL is only there to heal a dropped event.</para>
/// </summary>
    ...
    ILogger<BlockCache> logger)
{
    ...
    /// One Redis read each, then one bus round trip for the remainder.
    /// </summary>
    ...
    /// <summary>Drops one user's entry. A block touches two people, so the event handler evicts
    /// both sides.</summary>

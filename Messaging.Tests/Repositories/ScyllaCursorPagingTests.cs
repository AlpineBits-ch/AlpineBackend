...
/// <summary>
/// Message-history scroll paging (<see cref="ScyllaMessageRepository.GetMessagePageByCursorAsync"/>)
/// <b>against a real Scylla node</b> - the sibling of <c>ScyllaDmRetentionRangeDeleteTests</c>, and
/// for the same reason.
///
/// <para><b>Why this cannot be a fake-mapper test.</b> Every bug this fixture exists to catch lives
/// in the interaction between a <c>WHERE</c> relation and the <i>order the cluster scans in</i>.
/// <c>FakeCassandraMapper</c> records statement text; it has no clustering order, so a statement that
/// silently returns the wrong rows and a statement that returns the right ones are indistinguishable
/// to it. The bug being reproduced here produced no error, no warning and no exception on a live
/// node - only a page that was quietly missing a message.</para>
///
/// <para><b>The hazard.</b> <c>messages</c> is clustered <c>(created_at DESC, message_id ASC)</c> -
/// mixed directions. A CQL row-tuple relation <c>(created_at, message_id) &lt; (?, ?)</c> is plain
/// ascending-lexicographic on both components, but neither of the two orders this table can be read
/// in is. Read naturally the tie-break runs ASC while the timestamp runs DESC; read
/// <c>ORDER BY created_at ASC</c> the cluster reverses <b>both</b> columns, so the tie-break runs
/// DESC while the timestamp runs ASC. Either way the relation and the scan disagree about the
/// tie-break column, so a page cap landing inside a same-millisecond group takes the wrong members of
/// it and the next page - resuming from a row the scan had already passed - never returns the rest.
/// Same-millisecond groups are ordinary: bursts, bulk sends, bot output, imported history.</para>
///
/// <para>Skipped, loudly and by name, when no node is configured - the same shape as
/// <c>ScyllaDmRetentionRangeDeleteTests</c> and <c>ECHO_TEST_POSTGRES</c>, so CI without the
/// container still passes. Point <c>ECHO_TEST_SCYLLA</c> at a node (<c>host:port</c>, e.g.
/// <c>localhost:9042</c>) to run it. <c>ECHO_TEST_SCYLLA_USERNAME</c>/<c>_PASSWORD</c> are optional
/// and only needed for a node started with PasswordAuthenticator, as compose.yaml's is.</para>
/// </summary>
[TestFixture]
[Category("Scylla")]
public class ScyllaCursorPagingTests
{
    ...
    public async Task TheOriginalRowTupleCursorIsAcceptedByTheClusterAndSilentlyLosesRows_ScrollingForward()
    {
        ...
        // for ORDER BY created_at ASC, which on a mixed-order table reverses BOTH clustering columns,
        // so the scan ran (created_at ASC, message_id DESC) while the relation stayed
        // ascending-lexicographic. Inside a millisecond the scan again handed back the furthest
        // members first and LIMIT kept those.
        var contextId = NewContextId();
        var instant = Now - TimeSpan.FromMinutes(5);
        var group = await SeedInstantGroupAsync(contextId, instant, 4);
    ...
    {
        // The symptom this fixture is named for: a user scrolls up through a burst and one message is
        // simply not there. Page size 2 against a group of 7 forces the boundary inside the group
        // three times over.
        var contextId = NewContextId();
        var instant = Now - TimeSpan.FromMinutes(5);
    ...
    {
        // "Adjacent" is the entire contract of a cursor page. Returning the right *set* of rows while
        // getting adjacency wrong still loses messages the moment the client scrolls again, so this
        // asserts position rather than membership.
        var contextId = NewContextId();
        var instant = Now - TimeSpan.FromMinutes(5);
    ...
    {
        // Deliberately unlike the retention scan, which over-returns to keep whole millisecond groups
        // together. History paging cannot do that - the limit is a client's page size - and it does
        // not have to: the resume cursor here is a message *id*, so a group split across two pages is
        // picked up exactly where it was left.
        var contextId = NewContextId();
        var instant = Now - TimeSpan.FromMinutes(5);
    ...
    {
        // The order the relational backend returns (OrderBy CreatedAt, ThenBy Id) - asserted here so
        // a client that pages one backend and then the other cannot see two different orders. The
        // comparison is ordinal, which is what a Scylla clustering key on a text column performs.
        var contextId = NewContextId();
    ...
    {
        // Scrolling up from the oldest message in a partition, and down from the newest. A cursor
        // that compared wrongly could return the far end of the partition here instead of nothing.
        var contextId = NewContextId();
    ...
    {
        // GetMessagesByContextIdAsync reads `skip + take` rows and drops `skip`. The scan's
        // tie-break is message_id ASC while newest-first wants DESC, so a LIMIT landing inside a
        // millisecond kept the wrong members of that group - and a message could be missing from
        // page 1 and from page 2 both. Offset paging still drifts under concurrent writes, which is
        // why it is deprecated; it should not also lose rows on a partition nobody is writing to.
        var contextId = NewContextId();

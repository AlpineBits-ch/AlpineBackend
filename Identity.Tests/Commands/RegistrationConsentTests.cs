...
/// <summary>
/// T1-10: registration records consent for the then-current Terms and Privacy versions, with the IP.
///
/// <para>Asserted at the handler rather than over HTTP because the address is the point, and Alba
/// hosts the app over an in-process TestServer with no remote endpoint - so the one thing worth
/// proving is invisible there. What this covers is the last hop: the id the controller read off the
/// connection reaching the consent row unchanged.</para>
///
/// <para>Wolverine handler, so no SaveChangesAsync inside it; the tests commit themselves to stand in
/// for the transactional middleware.</para>
/// </summary>
[TestFixture]

...
/// <summary>
/// Hand-rolled no-op IMessageBus for DiscordStructureSyncHandler tests - mirrors this repo's
/// no-mocking-framework convention (Guild.Tests/Helpers/FakeMessageBus.cs). Only the two
/// InvokeAsync overloads the handler actually calls (generic upsert, non-generic delete) do
/// anything; everything else throws since nothing in this suite should reach it.
/// </summary>
public class FakeSyncMessageBus : IMessageBus
{
    ...
    public Task<T> InvokeAsync<T>(object message, CancellationToken cancellation = default, TimeSpan? timeout = null)
    {
        ...
        {
            UpsertChannelFromSyncCommand => new UpsertChannelFromSyncResponse { EchoId = $"echo-chan-{_idCounter}" },
            UpsertRoleFromSyncCommand => new UpsertRoleFromSyncResponse { EchoId = $"echo-role-{_idCounter}" },
            _ => throw new NotImplementedException($"FakeSyncMessageBus has no canned response for {message.GetType().Name}"),
        };

...
/// (privacy spec T2-17).
///
/// <para>The resolver applies no visibility setting - that is <c>ProfileProjectionService</c>'s job,
/// and <c>ProfileControllerPrivacyTests</c> is where it is asserted. What matters here is that the
/// contract is read correctly, that the request is the batched one, and above all that an Identity
/// outage produces "nothing to show" rather than an exception on the profile-read path.</para>
/// </summary>
[TestFixture]

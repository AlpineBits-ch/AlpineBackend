...
public class MessagingEndpointsTests
{
    ...
    /// <summary>Stands in for CreateMessageCommandHandler. The endpoint only reads the stored
    /// message back to shape its 201 response - the real handler is what raises the MessageCreated
    /// for the send, which CreateMessageEventFanOutTests covers against the real handler.</summary>
    ...
    /// command, because that handler - not this endpoint - is what raises MessageCreated. A mention
    /// flag dropped here is a ping that never leaves the building; the event-side of the same
    /// contract is asserted in CreateMessageCommandTests.
    /// </summary>
    [Test]
    ...
    // yet heard encryption was turned on would otherwise post readable content into a room everyone
    // believes is end-to-end encrypted - silently, and irreversibly once stored.
    // ══════════════════════════════════════════════════════════════════════════

...
/// <summary>
/// Shapes of real Discord API v10 JSON responses (REST) and Gateway dispatch payloads this
/// service needs. Distinct from Bots.Contracts' payloads, which represent Echo-guild-shaped-as-
/// Discord data for outbound bot consumption (the opposite direction) - these represent Discord's
/// own actual data being parsed inbound.
/// </summary>
...
public class DiscordOverwritePayload
{
    ...
    /// <summary>0 = role, 1 = member. Member-targeted overwrites are dropped by
    /// DiscordChannelTypeMapper/the import handler - no Echo member exists to attach them to.</summary>
    [JsonPropertyName("type")]
    public int Type { get; set; }
...
/// <summary>GET /guilds/{id}/onboarding. Prompt options carry Discord role/channel ids, which the
/// import remaps to the freshly-created Echo ids before applying.</summary>
